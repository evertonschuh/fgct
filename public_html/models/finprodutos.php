...
class IntranetModelFinProdutos extends JModelList
{
	var $_db = null;
	var $_user = null;
	var $_app = null;	
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        $this->context .'list.ordering','filter_order', 'id_produto', 'string');
    ...
    protected function getListQuery() {
		...
		$query->select($this->_db->quoteName(  array('id_produto',
													 'status_produto',
													 'name_produto',
													 'checked_out',
													 'checked_out_time',
        ...
        if ($status!='')
			 $query->where( $this->_db->quoteName('status_produto') . '=' . $this->_db->escape( $status ) );
			
		$search = $this->getState('filter.search');
        if ($search!='')
			 $query->where( $this->_db->quoteName('name_produto') . ' LIKE \'%' . $this->_db->escape($search) . '%\'' );
	...
	function setPublish(  $cid, $status ){
		if (count( $cid )) 
		{
			...
			$fields = array( $this->_db->quoteName('status_produto') . ' = ' . $this->_db->quote( $status ) );

			$conditions = array( $this->_db->quoteName('id_produto') . ' IN (' . $cids . ')' );
			
			$query->update($this->_db->quoteName('#__intranet_produto'))->set($fields)->where($conditions);
