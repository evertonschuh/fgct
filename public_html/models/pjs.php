...
class EASistemasModelPjs extends JModelList
{
	var $_db = null;
	var $_user = null;
	var $_app = null;
	...
	protected function getListQuery()
	{
		...
		$query->select($this->_db->quoteName(array(
			...
			'sigla_estado',
			'#__pj.checked_out',
			'#__pj.checked_out_time',
		)));

		$query->select('A.name AS name_register_pj');
		$query->select('B.name AS name_update_pj');

		$query->from($this->_db->quoteName('#__pj'));
		$query->innerJoin($this->_db->quoteName('#__estado') . ' USING (' . $this->_db->quoteName('id_estado') . ')');
		$query->innerJoin($this->_db->quoteName('#__cidade') . ' USING (' . $this->_db->quoteName('id_estado') . ',' . $this->_db->quoteName('id_cidade') . ')');
		...
		*/
		...
		if ($search != '') {
			...
			$searches	= array();
			$searches[]	= 'razao_social_pj LIKE ' . $token;
	...
	function remove_pj()
	{
		...
		if (count($cid)) {
			...
			$query->delete($this->_db->quoteName('#__pj'));
	...
	function setPublish($cid, $status)
	{
		if (count($cid)) {
			...
			$query->update($this->_db->quoteName('#__pj'))->set($fields)->where($conditions);
