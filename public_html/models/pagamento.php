...
class EASistemasModelPagamento extends JModel 
{
	...
	function __construct()
	{
		...
		if (!$this->isCheckedOut() ) {
			...
			$link = 'index.php?view=pagamentos';
	...
	function getAnuidadePagamento()
	{
		...
		$query->from($this->_db->quoteName('#__anuidade'));
		$query->order('text');
		$this->_db->setQuery($query);
		return $this->_db->loadObjectList();	
	}
	...
	function getProdutoPagamento()
	{
		...
		$query->from($this->_db->quoteName('#__produto'));
		$query->order('text');
		$this->_db->setQuery($query);
		return $this->_db->loadObjectList();	
	}
	...
	function getValues() 
	{
		...
		$query->where( $this->_db->quoteName('#__pagamento.id_pagamento') . '=' . $this->_db->quote( $this->_id ) );
	...
	function store() 
	{
		...
		if($this->_id):
			...
			JLog::add($this->_user->get('id') . JText::_('		Pagamento Editado -  idPg('.$this->_id.')'), JLog::INFO, 'pagamento');
		else:
			$this->setId( $row->get('id_pagamento') ); 	
			JLog::add($this->_user->get('id') . JText::_('		Pagamento Cadastrado -  idPg('.$this->_id.')'), JLog::INFO, 'pagamento');
		endif;
