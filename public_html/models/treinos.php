...
class EASistemasModelTreinos extends JModelList
{
	var $_db = null;
	var $_app = null;
	...
	protected function getListQuery()
	{
		...
		$query->from($this->_db->quoteName('#__treino'));

		$status = $this->getState('filter.status');
		if ($status != '')
			$query->where($this->_db->quoteName('status_treino') . '=' . $this->_db->quote($this->_db->escape($status)));
		else
			$query->where($this->_db->quoteName('status_treino') . '>=' . $this->_db->quote($this->_db->escape('0')));
		...
		if ($search != '') {
			...
			$searches	= array();
			$searches[]	= 'name_treino LIKE ' . $token;
	...
	public function getItems()
	{
		...
		if (empty($this->cache[$store]))
		{
			...
			{
				$item->total_treino = $this->_getTotalCorredores($item->id_treino);
			}
			$this->cache[$store] = $items;
		}
	...
	function _getTotalCorredores($id_treino)
	{
		/*
		$query = $this->_db->getQuery(true);	
		$query->select( 'COUNT(DISTINCT(' . $this->_db->quoteName('id_corredor') . '))' );	
		$query->from($this->_db->quoteName('#__corredor'));
		$query->where($this->_db->quoteName('id_treino') . ' = ' .  $this->_db->quote( $id_treino ));
	...
	function remove()
	{
		...
		if (count($cids)) {
			...
			$query = $this->_db->getQuery(true);
			$conditions = array($this->_db->quoteName('id_treino') . ' IN (' . $cid . ')');
			$query->delete($this->_db->quoteName('#__treino'));
	...
	function setPublish($cid, $status)
	{
		if (count($cid)) {
			...
			$fields = array($this->_db->quoteName('status_treino') . ' = ' . $this->_db->quote($status));

			$conditions = array($this->_db->quoteName('id_treino') . ' IN (' . $cids . ')');

			$query->update($this->_db->quoteName('#__treino'))->set($fields)->where($conditions);
	...
	{
		$query = $this->_db->getQuery(true);
		$query->select('id_treino as value, name_treino as text');
		$query->from('#__treino');
		$query->where( $this->_db->quoteName('status_treino') . '=' . $this->_db->escape( '1' ));
		$query->order( $this->_db->quoteName('name_treino') );
		$this->_db->setQuery($query);
		return $this->_db->loadObjectList();
	}
