...
class IntranetModelFinInscricaos extends JModelList
{
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		*/
        $ano = $this->_app->getUserStateFromRequest(
        $this->context .'filter.ano', 'ano', null,'string');
        $this->setState('filter.ano', $ano);
		/*
...
*/
    ...
    protected function getListQuery() {
		...
		$query->select( 'GROUP_CONCAT(DISTINCT name_campeonato ORDER BY name_campeonato ASC SEPARATOR \'<br/> \') as name_campeonato');
		$query->select( 'MAX(id_pagamento) AS id_pagamento');
		$query->select( 'CONCAT(GROUP_CONCAT(id_etapa), \'-\', GROUP_CONCAT(id_campeonato), \'-\', id_local) AS id_contabel');	
        ...
        if ($ano!='')
			$query->where( $this->_db->quoteName('ano_campeonato') . '=' .$this->_db->quote($ano) );
		...
		//$query->group( $this->_db->quoteName('name_etapa') );
		$query->group( $this->_db->quoteName('data_beg_etapa') );
		$query->group( $this->_db->quoteName('data_end_etapa') );		
		$query->group( $this->_db->quoteName('name_etapa') );
	...
	function getAnos()
	{
		...
		$query = $this->_db->getQuery(true);	
		$query->select('ListaInscricoes.ano_campeonato AS text, ListaInscricoes.ano_campeonato as value');
		$query->from('(' . $querylist . ') AS ListaInscricoes');
		$query->group( $this->_db->quoteName('ano_campeonato') );
