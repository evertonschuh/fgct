...
class IntranetModelFinPagamentos extends JModelList
{
	var $_db = null;
	var $_user = null;
	var $_app = null;	
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        $this->context .'list.ordering','filter_order', '#__intranet_pagamento.id_pagamento', 'string');
    ...
    protected function getListQuery() {
        ...
        if ($search!='') {	 
			...
			$searches	= array();
			$searches[]	= '#__intranet_pagamento.id_pagamento LIKE '.$token;
        ...
        if ($typesocio!='')
			switch($typesocio){
				case '1':
					$query->where( $this->_db->quoteName('#__intranet_pf.id_pf') . 'IS NOT NULL');
				break;
				case '2':
					$query->where( $this->_db->quoteName('#__intranet_pf.id_pf') . 'IS NOT NULL');
					$query->where( $this->_db->quoteName('#__intranet_pf.compressed_air_pf') . '='. $this->_db->quote('1'));
				break;
				case '3':
					$query->where( $this->_db->quoteName('#__intranet_pf.id_pf') . 'IS NOT NULL');
					$query->where( $this->_db->quoteName('#__intranet_pf.compressed_air_pf') . '='. $this->_db->quote('0'));
				break;
				case '4':
					$query->where( $this->_db->quoteName('#__intranet_pj.id_pj') . 'IS NOT NULL');
				break;
	...
	/*
			...
			$query->where( $this->_db->quoteName('#__intranet_pagamento.id_pagamento') .  ' IN (' . $cids . ')' );
			$this->_db->setQuery($query);
...
*/
    ...
    function getReport() {
		...
		$query->from('(' . $querylist . ') AS Relatorio');
		$query->innerJoin($this->_db->quoteName('#__intranet_associado') . ' AS Associado USING ('.$this->_db->quoteName('id_user').')');
		$query->leftJoin($this->_db->quoteName('#__intranet_pf') . ' AS Atleta USING ('.$this->_db->quoteName('id_user').')');
	...
	function getVTotalP()
	{
		...
		$query = $this->_db->getQuery(true);	
		$query->select('SUM(ListaPagamentos.valor_pago_pagamento)');
		$query->from('(' . $querylist . ') AS ListaPagamentos');
		$this->_db->setQuery($query);
		return $this->_db->loadResult();
	}
	...
	{	
		...
		{				
			...
			$query->delete( $this->_db->quoteName('#__intranet_pagamento') );
			...
			foreach($cids as $item)
				JLog::add($this->_user->get('id') . JText::_('		Pagamento Removido -  idpagamento('.$item.')'), JLog::INFO, 'finpagamentos');
