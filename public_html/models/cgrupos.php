...
class EASistemasModelCGrupos extends JModelList
{
	var $_db = null;
	var $_app = null;
	...
	protected function getListQuery()
	{
		...
		$query->select($this->_db->quoteName(array(
			'id_corredor_grupo',
			'status_corredor_grupo',
			'name_corredor_grupo',
			'checked_out',
			'checked_out_time',
		)));

		$query->from($this->_db->quoteName('#__corredor_grupo'));

		$status = $this->getState('filter.status');
		if ($status != '')
			$query->where($this->_db->quoteName('status_corredor_grupo') . '=' . $this->_db->quote($this->_db->escape($status)));
		else
			$query->where($this->_db->quoteName('status_corredor_grupo') . '>=' . $this->_db->quote($this->_db->escape('0')));
		...
		if ($search != '') {
			...
			$searches	= array();
			$searches[]	= 'name_corredor_grupo LIKE ' . $token;
...
/*
			...
			{
				$id_users[] = (int) $item->id_corredor_grupos;
				$item->group_count = 0;
				$item->group_names = '';
				$item->note_count = 0;
			...
			*/
			...
			foreach ($items as &$item)
			{
				...
				//{
					$item->total_corredor_grupo = $this->_getTotalCorredores($item->id_corredor_grupo);
	...
	function _getTotalCorredores($id_corredor_grupo)
	{
		$query = $this->_db->getQuery(true);	
		$query->select( 'COUNT(DISTINCT(' . $this->_db->quoteName('id_corredor') . '))' );	
		$query->from($this->_db->quoteName('#__corredor'));
		$query->where($this->_db->quoteName('id_corredor_grupo') . ' = ' .  $this->_db->quote( $id_corredor_grupo ));
		$this->_db->setQuery($query);
	...
	function remove()
	{
		...
		if (count($cids)) {
			...
			$query = $this->_db->getQuery(true);
			$conditions = array($this->_db->quoteName('id_corredor_grupo') . ' IN (' . $cid . ')');
			$query->delete($this->_db->quoteName('#__corredor_grupo'));
	...
	function setPublish($cid, $status)
	{
		if (count($cid)) {
			...
			$fields = array($this->_db->quoteName('status_corredor_grupo') . ' = ' . $this->_db->quote($status));

			$conditions = array($this->_db->quoteName('id_corredor_grupo') . ' IN (' . $cids . ')');

			$query->update($this->_db->quoteName('#__corredor_grupo'))->set($fields)->where($conditions);
