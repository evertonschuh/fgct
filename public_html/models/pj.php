...
class EASistemasModelPj extends JModel
{
	...
	function store()
	{
		...
		if (!empty($data['vencr_pj'])) {
			...
			$data['vencr_pj'] = implode("-", array_reverse(explode("/", $dataaTmp[0]))) . ' ' . $dataaTmp[1];
			$data['vencr_pj'] = JFactory::getDate($data['vencr_pj'], $siteOffset)->toFormat('%Y-%m-%d', true);
		} else
			$data['vencr_pj'] = NULL;
		...
		JLog::addLogger(array('text_file' => 'log.pj.php'));

		if ($this->_id) :
			$row->checkin($this->_id);
			JLog::add($this->_user->get('id') . JText::_('		Pessoa Jurídica Editada -  idPJ(' . $this->_id . ')'), JLog::INFO, 'pj');
		else :
			$this->setId($row->get('id_pj'));
			JLog::add($this->_user->get('id') . JText::_('		Pessoa Jurídica Cadastrada -  idPJ(' . $this->_id . ')'), JLog::INFO, 'pj');
		endif;
