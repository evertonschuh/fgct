<?php
/**
* @version		$Id: Embed Google Docs Viewer v1.4.2 2015-03-21 16:37 $
* @package		Joomla 1.6
...
class plgContentembed_google_docs_viewer extends JPlugin
{
	...
	function onContentPrepare($row, $params, $limitstart)
	{	
		...
		if ( $found )
		{
			...
			{			
				...
				$replacement[$count] = "\n<iframe width='$width' height='$height' style='border: ".$border."px; min-height:800px;  $border_style $border_color' ";
                if(preg_match($pattern_google_docs, $doc_path)) {						
						...
						*/
						if(preg_match('/\/document\//i', $doc_path)) {
							$doc_path = preg_replace('/(\/edit|\/preview).*$/', '',$doc_path);
							$doc_path = preg_replace('/\/$/', '',$doc_path);
							if($mode == 1) {
								$doc_path = str_replace('pub?id=', 'd/',$doc_path);							
