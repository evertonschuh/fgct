...
final class Util
{
    ...
    public static function fatorVencimento($date, $format = '%Y-%m-%d')
    {
        ...
        $base = new Carbon('1997-10-07');
		
	    return $base->diffInDays($date);
		/*
		$config   = JFactory::getConfig();
		$siteOffset = $config->getValue('offset');
		
		$date = new DateTime('1997-10-07');	
    	return $date->diff( JFactory::getDate($date, $siteOffset)->toFormat($format, true) )->days;
		*/
    }
    ...
    public static function file2array($file)
    {
        ...
        return array_map('utf8_encode', $aFile);
    }
    ...
    {
        if (is_subclass_of($obj, 'PessoaContract')) {
            $property = $obj;
            return $obj;
        } elseif (is_array($obj)) {
            $obj = new Pessoa($obj);
