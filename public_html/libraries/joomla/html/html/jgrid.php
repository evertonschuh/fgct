...
abstract class JHtmlJGrid
{
	...
	public static function published($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb', $publish_up = null, $publish_down = null)
	{
		...
		$states = array(1 => array('unpublish', 'JPUBLISHED', 'JLIB_HTML_UNPUBLISH_ITEM', 'JPUBLISHED', false, 'fa-eye', 'publish'),
			0 => array('publish', 'JUNPUBLISHED', 'JLIB_HTML_PUBLISH_ITEM', 'JUNPUBLISHED', false, 'fa-eye-slash', 'unpublish'),
			2 => array('unpublish', 'JARCHIVED', 'JLIB_HTML_UNPUBLISH_ITEM', 'JARCHIVED', false, 'fa-archive', 'archive'),
			-2 => array('publish', 'JTRASHED', 'JLIB_HTML_PUBLISH_ITEM', 'JTRASHED', false, 'fa-trash', 'trash'));
	...
	/**
	 * Returns a featured state on a grid
	 *
	 * @param   integer       $value     The state value.
	 * @param   integer       $i         The row index
	 * @param   string|array  $prefix    An optional task prefix or an array of options
	 ...
	 * @since   11.1
	 */
	public static function featured($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb', $publish_up = null, $publish_down = null)
	{
		...
		$states	= array(
			0	=> array('unfeatured.png',	'featured',		'JLIB_HTML_UNFEATURED_ITEM',	'JLIB_HTML_FEATURED_ITEM'),
			1	=> array('featured.png',	'unfeatured',	'JLIB_HTML_FEATURED_ITEM',		'JLIB_HTML_UNFEATURED_ITEM'),
		);
		$state	= JArrayHelper::getValue($states, (int) $value, $states[1]);
		$html	= JHtml::_('image', 'admin/'.$state[0], JText::_($state[2]), NULL, true);
		$html	= '<a href="#" onclick="return listItemTask(\''.$checkbox.$i.'\',\''.$state[1].'\')" title="'.JText::_($state[3]).'">' . $html.'</a>';
	...
	/**
	 * Returns a lock state on a grid
	 *
	 * @param   integer       $value     The state value.
	 * @param   integer       $i         The row index
	 * @param   string|array  $prefix    An optional task prefix or an array of options
	 ...
	 * @since   11.1
	 */
	public static function lock($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb', $publish_up = null, $publish_down = null)
	{
		...
		// Array of image, task, title, action
		$states	= array(
			0	=> array('lockpj.png',	'lockpj',		'JLIB_HTML_UNFEATURED_ITEM',	'JLIB_HTML_FEATURED_ITEM'),
			1	=> array('unlockpj.png', 'unlockpj',	'JLIB_HTML_FEATURED_ITEM',		'JLIB_HTML_UNFEATURED_ITEM'),
		);
		$state	= JArrayHelper::getValue($states, (int) $value, $states[1]);
		$html	= JHtml::_('image', 'admin/'.$state[0], JText::_($state[2]), NULL, true);
		$html	= '<a href="#" onclick="return listItemTask(\''.$checkbox.$i.'\',\''.$state[1].'\')" title="'.JText::_($state[3]).'">' . $html.'</a>';
	...
	public static function checkedout($i, $editorName, $time, $prefix = '', $enabled = false, $checkbox = 'cb')
	{
		...
		return self::action(
			$i, 'checkin', $prefix, JText::_('JLIB_HTML_CHECKED_OUT'), $active_title, $inactive_title, true, 'fa-lock',
	...
	public static function orderUp($i, $task = 'orderup', $prefix = '', $text = 'JLIB_HTML_MOVE_UP', $enabled = true, $checkbox = 'cb')
	{
		...
		return self::action($i, $task, $prefix, $text, $text, $text, false, 'fa-caret-up', 'uparrow_disabled', $enabled, true, $checkbox);
	}
	...
	public static function orderDown($i, $task = 'orderdown', $prefix = '', $text = 'JLIB_HTML_MOVE_DOWN', $enabled = true, $checkbox = 'cb')
	{
		...
		return self::action($i, $task, $prefix, $text, $text, $text, false, 'fa-caret-down', 'downarrow_disabled', $enabled, true, $checkbox);
	}
}
