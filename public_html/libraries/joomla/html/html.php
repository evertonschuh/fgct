...
abstract class JHtml
{
				...
				foreach ($potential as $strip)
				{
					...
					foreach ($files as $file)
					{
						...
						if (file_exists(JPATH_THEMES . "/$template/$folder/$file"))
						{
							...
							//file_exists(JPATH_ROOT . "/views/$extension/$element/$folder/$file"))
							
							//$includes[] = JURI::root(true, '/fbt') . "/views/$template/$folder/$file";
							$includes[] =  "/views/$template/$folder/$file";
						...
						else
						{
							...
							if (strpos($file, '/'))
							{
								...
								if (strpos($file, '/'))
								{
									...
									if (file_exists(JPATH_BASE . "/views/$extension/$element/$folder/$file"))
									{
										$includes[] = JURI::root(true) . "/views/$extension/$element/$folder/$file";
										break;
									}
									// Try to deal with classical file in a a media subfolder called element
									elseif (file_exists(JPATH_BASE . "/views/$extension/$folder/$element/$file"))
									{
										$includes[] = JURI::root(true) . "/views/$extension/$folder/$element/$file";
										break;
									}
									// Try to deal with system files in the template folder
									elseif (file_exists(JPATH_BASE . "/$template/$folder/system/$element/$file"))
									{
										$includes[] = JURI::root(true) . "/views/$template/$folder/system/$element/$file";
										break;
									}
									// Try to deal with system files in the media folder
									elseif (file_exists(JPATH_BASE . "/views/system/$folder/$element/$file"))
									{
										$includes[] = JURI::root(true) . "/views/system/$folder/$element/$file";
										break;
									}
									// inclusao paa O Emprego
									elseif (file_exists(JPATH_BASE . "/views/$template/$folder/$file"))
									{

										
										$includes[] = JURI::root(true) . "/views/$template/$folder/$file";
										break;
									}
									
								}
								// Try to deals in the extension media folder
								elseif (file_exists(JPATH_BASE . "/views/$extension/$folder/$file"))
								{
									$includes[] = JURI::root(true) . "/views/$extension/$folder/$file";
									break;
								}
								// Try to deal with system files in the template folder
								elseif (file_exists(JPATH_BASE . "/views/$template/$folder/system/$file"))
								{
									$includes[] = JURI::root(true) . "/views/$template/$folder/system/$file";
									break;
								}
								// Try to deal with system files in the media folder
								elseif (file_exists(JPATH_BASE . "/views/system/$folder/$file"))
								{
									$includes[] = JURI::root(true) . "/views/system/$folder/$file";
									break;
								}
								// Try to deal with system files in the media folder
								elseif (file_exists(JPATH_BASE . "/views/$template/$folder/$file"))
							...
							// Try to deal with system files in the media folder
							elseif (file_exists(JPATH_BASE . "/views/system/$file"))
							{
								$includes[] = JURI::root(true) . "/views/system/$file";
								break;
							}							
							// Try to deal with system files in the media folder
							elseif (file_exists(JPATH_BASE . $file))
							{
	...
	public static function script($file, $framework = false, $relative = false, $path_only = false, $detect_browser = true, $detect_debug = true)
	{
		...
		//echo $file . ' - '. $relative . ' - '. $detect_browser . ' - '. $detect_debug ; 
		//print_r($includes);
		//echo '1 <br/>';
		// If only path is required
		if ($path_only)
