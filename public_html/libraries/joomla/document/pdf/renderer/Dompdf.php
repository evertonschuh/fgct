...
if(!file_exists(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Dompdf.php')){
	JError::raiseError(500, JText::sprintf('JLIB_APPLICATION_ERROR_APPLICATION_LOAD','DOMPDF'));
} else {
	
	if(!class_exists('Dompdf')){
		//require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Autoloader.php');	
...
class JDocumentRendererDompdf extends Dompdf
{
	...
	public function __construct(&$doc)
	{
		...
		$this->_doc->orientation = isset($doc->orientation) ? $doc->orientation	: 'P';
		$this->_doc->unit		 = isset($doc->unit) 		? $doc->unit 		: 'mm';
		$this->_doc->format 	 = isset($doc->format) 		? $doc->format 		: 'A4';
		$this->_doc->unicode	 = isset($doc->unicode) 	? $doc->unicode		: true;
		$this->_doc->pdfa		 = isset($doc->pdfa) 		? $doc->pdfa		: false; 
  ...
  <div id="footer">
	...
	public function render($name = '', $destination = 'I', $content = null)
	{
		...
		// $app = JFactory::getApplication();
				...
				if($this->_doc->orientation != 'P' )
				$this->set_paper($this->_doc->format , $this->_doc->orientation);
                //echo $data;exit;
	...
	private function fullPaths($data)
	{
		...
		//@page { margin: 180px 50px; }
		  ...
		  <style>
			...
			#HTMLHeader { position: fixed; padding-left:'.$this->jdoc->_margin_left.'mm; padding-right:'.$this->jdoc->_margin_right.'mm; padding-top:'.$this->jdoc->_margin_header.'mm; left: 0px; top: 0px; right: 0px; height: 10px; text-align: right;}
