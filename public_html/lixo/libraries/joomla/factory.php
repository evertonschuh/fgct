...
abstract class JFactory
{
	...
	/*
	Function Ineje Param
	*/
	
	public static function setParam( $mainframe=null )
	{
		if ($mainframe ===null) {
			$mainframe = $this->getApplication('site');
		}
		
		$mainframe->language = 'pt-BR';
		$mainframe->setTemplate('system');

		$config = new JConfig();
		
		$document = JFactory::getDocument();
		$document->setTitle($config->sitetitle);
		$document->setDescription($config->MetaDesc);
		$document->setBase = $config->urlbase;
		$document->setCanonical(JURI::current()); 
			
		$document->MetaAuthor  = $config->MetaAuthor;
		$document->MetaFace  = $config->MetaFace;
