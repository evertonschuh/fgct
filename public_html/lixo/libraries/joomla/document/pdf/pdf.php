...
class JDocumentPDF extends JDocument
{
	...
	function __construct($options = array())
	{
		...
		//JFactory::getApplication()->input->set('view','certificado');
		...
		$option = $input->get('view','','word');
		$view = $input->get('view',substr($option, 4),'word');
		$app = JFactory::getApplication();
		

		if (!$app->isSite()) $viewPath .= 'admin/';
		$viewPath .= 'views/'.$view.'/view.pdf.php';
	...
	function setPaper($paper = 'A4', $orientation = 'P') {
		$this->paper = $paper;
	...
	if(version_compare(JVERSION,'3.0.0','ge')) {
		...
		$pdf->set_paper($this->paper , $this->orientation);
