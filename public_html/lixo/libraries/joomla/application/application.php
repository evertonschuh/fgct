...
class JApplication extends JObject
{
	...
	{
		if (empty($component))  $component='com_easistemas';
		
		$component='com_easistemas';
	 ...
	 * @since   11.1
	 */
	public function getUserStateFromRequest($key, $request = null, $default = null, $type = 'none')
	{
	...
	public function login($credentials, $options = array())
	{
		...
		if ($response->status === JAuthentication::STATUS_SUCCESS)
		{
			...
			JError::raiseWarning('SOME_ERROR_CODE', JText::_('EASISTEMAS_CONTROLLER_LOGIN_ERROR'));
			return false;
		}
	...
	public function getSef()
	{
		...
		$query->select(array('name_sef','alias_sef', 'name_sef_type'));
		$query->from('#__sef');
		$query->innerJoin('#__sef_type USING (id_sef_type)');
		$query->where('status_sef_type = 1 ');
		$query->where('status_sef = 1 ');
		
		// Set the query
		$db->setQuery($query);
		if (!($sef = $db->loadObjectList())) {
