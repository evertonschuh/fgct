...
abstract class JHtmlSelect
{
	...
	public static function option($value, $text = '', $optKey = 'value', $optText = 'text', $disable = false)
	{
		...
		$optionkey = $options['option.key'];
		$optiontext = $options['option.text'];
		
		$obj->$optionkey  = $value;
		$obj->$optiontext = trim($text) ? $text : $value;
		...
		elseif ($hasProperty)
		{
			$optionlabel = $options['option.label'];
			$obj->$optionlabel = '';
		}
		...
		{
			$optionattr = $options['option.attr'];
			$obj->$optionattr = $options['attr'];
		}

		// Set disable only if it has a property and a value
		if ($options['disable'] !== null)
		{
			$optiondisable = $options['option.disable'];
			$obj->$optiondisable = $options['disable'];
		}
		return $obj;
	}
	...
	public static function options($arr, $optKey = 'value', $optText = 'text', $selected = null, $translate = false)
	{
		...
		foreach ($arr as $elementKey => $element)
		{
			...
			{
				$optionkey = $options['option.key'];
				$key = $optionkey === null ? $elementKey : $element->$optionkey;
				
				$optiontext = $options['option.text'];
				$text = $element->$optiontext;
				
				$optionattr = $options['option.attr'];
				if (isset($element->$optionattr))
				{
					$attr = $element->$optionattr;
				}
				
				$optionid = $options['option.id'];
				if (isset($element->$optionid))
				{
					$id = $element->$optionid;
				}
				
				$optionlabel = $options['option.label'];
				if (isset($element->$optionlabel))
				{
					$label = $element->$optionlabel;
				}
				
				$optiondisable = $options['option.disable'];
				if (isset($element->$optiondisable) && $element->$optiondisable)
				{
					$extra .= ' disabled="disabled"';
				}
			}
		...
		$translate = false)
	{
		...
		foreach ($data as $i => $obj)
		{
			...
			$html .= "\n\t"  . '<label for="' . $id_text . $k . '"' . ' id="' . $id_text . $k . '-lbl" class="form-check-label radiobtn">'. '<input type="radio" name="' . $name . '"' . ' id="' . $id_text . $i . '" value="' . $k . '"' . ' ' . $extra . ' '
	...
	function checkboxlist($data, $name, $attribs = null, $optKey = 'value', $optText = 'text', $selected = null, $idtag = false,
		$translate = false)
	{
		...
		foreach ($data as $i => $obj)
		{
			...
			$html .= "\n\t"  . '<label for="' . $id_text . $k . '"' . ' id="' . $id_text . $k . '-lbl" class="checkboxbtn">'. '<input type="checkbox" name="' . $name . '[]"' . ' id="' . $id_text . $i . '" value="' . $k . '"' . ' ' . $extra . ' '
				. $attribs . '/>' . "\n\t" . $t
				. '</label>';
		}
