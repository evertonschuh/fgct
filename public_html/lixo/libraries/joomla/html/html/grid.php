...
abstract class JHtmlGrid
{
	...
	public static function sort($title, $order, $direction = 'asc', $selected = 0, $task = null, $new_direction = 'asc')
	{
		...
		$images = array('sort_asc_table.png', 'sort_desc_table.png');
		$icons = array('fas fa-sort-amount-down-alt', 'fas fa-sort-amount-up');
		...
		if ($order == $selected)
		{
			$html .= '<i class="fa ' . $icons[$index] . '" aria-hidden="true"></i>';

			//$html .= JHtml::_('image', 'admin/' . $images[$index], '', null, true);
		}
	 ...
	 * @since   11.1
	 */
	public static function block($value, $i, $img1 = 'fa-times', $img0 = 'fa-check', $prefix = '')
	{
		if (is_object($value))
		{
			$value = $value->block;
		...
		$img = $value ? $img1 : $img0;
		$task = $value ? 'unblock' : 'block';
		$alt = $value ? JText::_('JLOCKED') : JText::_('JUNLOCKED');
		$action = $value ? JText::_('JLIB_HTML_UNBLOCKED_ITEM') : JText::_('JLIB_HTML_BLOCKED_ITEM');
		/*
		$href = '
		...
		*/
		$href = '
		<a href="javascript:void(0);" class="grid-icon" onclick="return listItemTask(\'cb' . $i . '\',\'' . $prefix . $task . '\')" title="' . $action . '"><i class="fa '.$img .'"></i></a>';
		return $href;
	}	
	 ...
	 * @since   11.1
	 */
	public static function lock($value, $i, $img1 = 'fa-lock', $img0 = 'fa-unlock', $prefix = '')
	{
		if (is_object($value))
		{
			$value = $value->lock;
		...
		$img = $value ? $img1 : $img0;
		$task = $value ? 'unlock' : 'lock';
		$alt = $value ? JText::_('JLOCKED') : JText::_('JUNLOCKED');
		$action = $value ? JText::_('JLIB_HTML_UNLOCKED_ITEM') : JText::_('JLIB_HTML_LOCKED_ITEM');
		/*
		$href = '
		<a href="javascript:void(0);" onclick="return listItemTask(\'cb' . $i . '\',\'' . $prefix . $task . '\')" title="' . $action . '">'
			. JHtml::_('image', 'admin/' . $img, $alt, null, true) . '</a>';
	 ...
	 * @since   11.1
	 */
	public static function published($value, $i, $img1 = 'fa-eye', $img0 = 'fa-eye-slash', $prefix = '')
	{
		if (is_object($value))
		{
	 ...
	 * @since   11.1
	 */
	public static function featured($value, $i, $img1 = 'fas fa-check-circle', $img0 = 'fas fa-ban', $prefix = '')
	{
		...
		$alt = $value ? JText::_('JFEATURED') : JText::_('JUNFEATURED');
		$action = $value ? JText::_('Remover Definição') : JText::_('Definir como Padão');
/*
		$href = '
		<a href="javascript:void(0);" onclick="return listItemTask(\'cb' . $i . '\',\'' . $prefix . $task . '\')" title="' . $action . '">'
			. JHtml::_('image', 'admin/' . $img, $alt, null, true) . '</a>';
*/
		$href = '
		<a href="javascript:void(0);" class="grid-icon" onclick="return listItemTask(\'cb' . $i . '\',\'' . $prefix . $task . '\')" title="' . $action . '"><i class="'.$img .'"></i></a>';
	 ...
	 * @since   11.1
	 */
	public static function custom($value, $i, $img1, $img0, $task1, $task0, $alt1, $alt0, $lib1, $lib0, $prefix = '')
	{
		...
		$img = $value ? $img1 : $img0;
		$task = $value ? $task1 : $task0;
		$alt = $value ? $alt1 : $alt0;
		$action = $value ? $lib1 : $lib0;
		$href = '
		<a href="javascript:void(0);" class="grid-icon" onclick="return listItemTask(\'cb' . $i . '\',\'' . $prefix . $task . '\')" title="' . $action . '"><i class="fa '.$img .'"></i></a>';
	...
	public static function order($rows, $image = 'filesave.png', $task = 'saveorder')
	{
		...
		*/
		$href = '<a href="javascript:saveorder(' . (count($rows) - 1) . ', \'' . $task . '\')" class="save-order" title="'
			. JText::_('JLIB_HTML_SAVE_ORDER') . '"><i class="fas fa-save" ></i></a>';
