...
require_once(JPATH_LIBRARIES . DS . 'boleto' . DS . 'Cnab' . DS .  'Retorno' . DS .  'Cnab240' .DS .'AbstractRetorno.php');
require_once(JPATH_LIBRARIES . DS . 'boleto' . DS . 'Contracts' . DS .  'Boleto' . DS .'Boleto.php');
require_once(JPATH_LIBRARIES . DS . 'boleto' . DS . 'Contracts' . DS .  'Cnab' . DS .'RetornoCnab240.php');
    ...
    protected function processarDetalhe(array $detalhe)
    {
        ...
        if ($this->getSegmentType($detalhe) == 'T') {
            $d->setOcorrencia($this->rem(16, 17, $detalhe))
                ->setOcorrenciaDescricao($this->array_get( $this->ocorrencias, $this->detalheAtual()->getOcorrencia(), 'Desconhecida'))
            ...
            */
            ...
            if ($d->hasOcorrencia('06', '09', '17')) {
                ...
                $error = Util::appendStrings(
                    $this->array_get($this->rejeicoes, $msgAdicional[0], ''),
                    $this->array_get($this->rejeicoes, $msgAdicional[1], ''),
                    $this->array_get($this->rejeicoes, $msgAdicional[2], ''),
                    $this->array_get($this->rejeicoes, $msgAdicional[3], ''),
                    $this->array_get($this->rejeicoes, $msgAdicional[4], '')
                );
                $d->setError($error);
            } else {
