...
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Common' . DS . 'Signer.php');
...
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Common' . DS . 'Strings.php');
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Common' . DS . 'TimeZoneByUF.php');
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Common' . DS . 'UFList.php');
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Common' . DS . 'Validator.php');


require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Core'. DS . 'Factories' . DS . 'Contingency.php');
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Core'. DS . 'Factories' . DS . 'ContingencyNFe.php');
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Core'. DS . 'Factories' . DS . 'Header.php');
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Core'. DS . 'Factories' . DS . 'QRCode.php');


require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Core' . DS . 'Common' . DS . 'Config.php');
...
class ToolsCommon
{
     ...
     * @var array
     */
    protected $canonical = array(true,false,null,null);
    /**
     ...
     * @var array
     */
    protected $soapnamespaces = array(
    ...
    {
        $permit = array(
            55 => array('SVCAN', 'SVCRS', 'EPEC', 'FSDA'),
            65 => array('FSDA', 'EPEC', 'OFFLINE'),
        );
     ...
     * @return array
     */
    public function canonicalOptions($opt = array(true,false,null,null))
    {
    ...
    protected function servico(
        ...
        if ($this->versao < '4.00') {
            $this->objHeader = new SoapHeader(
                $this->urlNamespace,
                'nfeCabecMsg',
                array('cUF' => $this->urlcUF, 'versaoDados' => $this->urlVersion)
            );
        }
    }
     ...
     * @return string
     */
    protected function sendRequest($request, array $parameters = array())
    {
        $this->checkSoap();
