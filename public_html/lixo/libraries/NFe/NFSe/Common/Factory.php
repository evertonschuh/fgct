...
class FactoryBase
{
    ...
    public function __construct(Certificate $certificate, $algorithm = OPENSSL_ALGO_SHA1)
    {
        ...
        $this->pathSchemes = JPATH_LIBRARIES . DS . 'NFe'. DS . 'NFSe' . DS . 'schemes';
    }
     ...
     * @return string
     */
    public function signer($content, $method, $mark = '', array $canonical = array(), $rootname = '')
    {
        $content = str_replace("\n", "", $content);
        if (empty($canonical)) {
            $canonical = array(false,false,null,null);
        }
