...
require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'League' . DS . 'Flysystem' . DS . 'Filesystem.php');
...
abstract class SoapBase implements SoapInterface
{
     ...
     * @var array
     */
    protected $prefixes = array(1 => 'soapenv', 2 => 'S');
    /**
     * @var Certificate
     */
    ...
    private function isCertificateExpired(Certificate $certificate = null)
    {
        if (!$this->disableCertValidation) {
            if (null !== $certificate && $certificate->isExpired()) {
				require_once(JPATH_LIBRARIES . DS . 'NFe'. DS . 'Common' . DS . 'Certificate' . DS . 'Exception' . DS . 'Expired.php');
     ...
     * @return string[]
     */
    public function setSoapPrefix($prefixes = array() )
    {
     ...
     * @return string
     */
    private function getStringAttributes($namespaces = array())
    {
		$envelopeAttributes = '';
