...
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Canvas.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'CanvasFactory.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Cellmap.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Exception.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'FontMetrics.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Frame.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Helpers.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'JavascriptEmbedder.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'LineBox.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Options.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'PhpEvaluator.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Renderer.php');

require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Adapter' .DS. 'CPDF.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Frame' .DS. 'Factory.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Frame' .DS. 'FrameTree.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Image' .DS. 'Cache.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Renderer' .DS. 'ListBullet.php');
require_once(JPATH_LIBRARIES .DS. 'Dompdf' .DS. 'src' .DS. 'Css' .DS. 'Stylesheet.php');
...
class Dompdf
{
    ...
    */
        ...
        if ($options->isFontSubsettingEnabled() && $canvas instanceof CPDF) {
            foreach ($this->tree->get_frames() as $frame) {
                ...
                if ($style->display === "list-item") {
                    $chars = ListBulletRenderer::get_counter_chars($style->list_style_type);
                    $canvas->register_string_subset($style->font_family, $chars);
                ...
                if ($frame->get_node()->nodeName == "dompdf_generated") {
                    ...
                    $chars = ListBulletRenderer::get_counter_chars('decimal');
                    $canvas->register_string_subset($style->font_family, $chars);
                    $chars = ListBulletRenderer::get_counter_chars('upper-alpha');
                    $canvas->register_string_subset($style->font_family, $chars);
                    $chars = ListBulletRenderer::get_counter_chars('lower-alpha');
                    $canvas->register_string_subset($style->font_family, $chars);
                    $chars = ListBulletRenderer::get_counter_chars('lower-greek');
                    $canvas->register_string_subset($style->font_family, $chars);
    ...
    public function getPaperSize($paperSize = null)
    {
        ...
        if (is_array($size)) {
            return $size;
        } else if (isset(CPDF::$PAPER_SIZES[mb_strtolower($size)])) {
            return CPDF::$PAPER_SIZES[mb_strtolower($size)];
        } else {
            return CPDF::$PAPER_SIZES["letter"];
        }
    }
