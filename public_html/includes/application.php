...
final class JSite extends JApplication
{
	...
	public function initialise($options = array())
	{
		...
		if (empty($options['language'])) {
			...
			$options['language'] = $params->get($client->name, $config->get('language', 'pt-BR'));
			*/
			$options['language'] =  $config->get('language', 'pt-BR');
		}
		...
		if (!JLanguage::exists($options['language'])) {
			...
			if (JLanguage::exists($lang)) {
				...
				$options['language'] = 'pt-BR'; // as a last ditch fail to english
			}
		}
			...
			switch($document->getType())
			{
				case 'html':
					...
					if ($router->getMode() == JROUTER_MODE_SEF) {
						...
						$document->setBase($this->getCfg('urlbase'));
					}
					break;
	...
	public function login($credentials, $options = array())
	{
		...
		if (!array_key_exists('entry_url', $options)) {
			$options['entry_url'] = JURI::base().'index.php?view=login&task=login';
		}
