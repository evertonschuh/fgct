...
class TorneiosClassesTiroabala {

	const addColsBeforeProva = 1;
	...
	{   
		...
		$document->addStyleSheet( 'administrator/components/com_torneios/classes/css/tiroabala.css?v=' . $token  );	
		...
		$extension_custon = $extension . '_tiroabala';
	...
	{		
		...
		$queryResultsRS->select( 'RANETAPA.special_value_etapa');																															//----
		...
		$queryResultsRF->select('special_value_etapa');																																//----
	...
	function getClassificacao( $options = array() )
	{
		...
		//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
																																															//----
		if(count($options['numero_etapas_final']) > 0):																																		//----
			$queryEtapaFinal = $this->_db->getQuery(true);																																	//----
			$queryEtapaFinal->select('GROUP_CONCAT(ResultsFinal.RF) AS RFinal');																											//----
			$queryEtapaFinal->select('GROUP_CONCAT(id_etapa) as id_etapaF');																												//----
			$queryEtapaFinal->select('GROUP_CONCAT(name_etapa SEPARATOR \'@@**@**@@\') as name_etapaF');																					//----
			$queryEtapaFinal->select('GROUP_CONCAT(id_local) as id_localF');																												//----
			$queryEtapaFinal->select('GROUP_CONCAT(name SEPARATOR \'@@**@**@@\') AS name_localF');																							//----
			//$queryEtapaFinal->select('#__ranking_etapa.ordering as orderingF');																											//----
			$queryEtapaFinal->select('GROUP_CONCAT(ResultsFinal.special_status_etapa) AS special_status_etapa');																			//----
			$queryEtapaFinal->select('GROUP_CONCAT(ResultsFinal.special_value_etapa) AS special_value_etapa');																				//----
			$queryEtapaFinal->select('id_inscricao');																																		//----
																																															//----
			$queryEtapaFinal->from('#__ranking_etapa');																																		//----
			$queryEtapaFinal->innerJoin( '#__ranking_inscricao_etapa USING( id_etapa )' );																									//----
			$queryEtapaFinal->innerJoin( '#__ranking_inscricao USING( id_inscricao, id_campeonato )' );																						//----
			$queryEtapaFinal->innerJoin( '#__ranking_campeonato USING( id_campeonato )' );																									//----
			$queryEtapaFinal->innerJoin( '#__users ON( id = id_local )' );																													//----
			$queryEtapaFinal->leftJoin( '('.$queryResultsRF.') AS ResultsFinal USING( id_etapa, id_inscricao )' );																			//----
																																															//----
			$queryEtapaFinal->where('ResultsFinal.special_status_etapa = ' . $this->_db->quote( '1' ));																						//----
			if(!empty($options['id_prova']))																																				//----
				$queryEtapaFinal->where('id_prova = ' . $this->_db->quote( $options['id_prova'] ));																							//----
																																															//----
			$queryEtapaFinal->order('#__ranking_etapa.ordering');																															//----
		...
		if(count($options['numero_etapas_final']) > 0)																																		//----
			...
			$queryPosition->select( 'NULL as RankFinal' );                                                                                                                                	//----
                                                                                                                                                                                            //----
        if($options['decimal_prova'] == 0)																									                                             	//----
		...
		if(count($options['numero_etapas_final']) > 0)																																		//----
			$queryPosition->select('ROUND((('.$RF .')/('.$options['results_prova'].'))' .																									//----
							'+(IF(ISNULL(ResultadoFinal.RFinal), 0, (ResultadoFinal.RFinal)*ResultadoFinal.special_value_etapa)),'.$decimal_prova_final.') AS RankRf');						//----
		else																																												//----
			$queryPosition->select('ROUND((('.$RF .')/('.$options['results_prova'].')),'.$decimal_prova_final.') AS RankRf');																//----
																																															//----
																																															//----
		...
		if(count($options['numero_etapas_final']) > 0) {
			$querySomaFinalMultiplica = '';
			for ($i = 1; $i <=count($options['numero_etapas_final']); $i++) { 

				if($querySomaFinalMultiplica != '')	
					$querySomaFinalMultiplica .= '+';	

				$querySomaFinalMultiplica .= '(IF(COUNT(Final.RFinal) > '.($i-1). ',' .
												'SUBSTRING_INDEX( SUBSTRING_INDEX( Final.RFinal, '
												. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1)

												* SUBSTRING_INDEX( SUBSTRING_INDEX( Final.special_value_etapa, '
												. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1)
												, 0)
												)';	
				...
				$query->select( 'IF(COUNT(Final.RFinal) > '.($i-1)
									. ', SUBSTRING_INDEX( SUBSTRING_INDEX( Final.RFinal, '
									. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), NULL) AS rAddColsFinal' . $i);		
								
				$query->select( 'IF(COUNT(Final.RFinal) > '.($i-1)
									. ', SUBSTRING_INDEX( SUBSTRING_INDEX( Final.id_etapaF, '
									. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), NULL) AS eAddColsFinal'.$i);	
						
				$query->select( 'IF(COUNT(Final.RFinal) > '.($i-1)
									. ', SUBSTRING_INDEX( SUBSTRING_INDEX( Final.name_etapaF, '
									. $this->_db->quote('@@**@**@@').', '.($i).'),'.$this->_db->quote('@@**@**@@').', -1), NULL) AS enAddColsFinal'.$i);	
											
				$query->select( 'IF(COUNT(Final.RFinal) > '.($i-1)
									. ', SUBSTRING_INDEX( SUBSTRING_INDEX( Final.id_localF, '
									. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), NULL) AS lAddColsFinal'.$i);	
											
				$query->select( 'IF(COUNT(Final.RFinal) > '.($i-1)
									. ', SUBSTRING_INDEX( SUBSTRING_INDEX( Final.name_localF, '
									. $this->_db->quote('@@**@**@@').', '.$i.'),'.$this->_db->quote('@@**@**@@').', -1), NULL) AS lnAddColsFinal'.$i);	
			}
			$query->select('ROUND((('.$querySomaFinal.'))+('.$querySomaFinalMultiplica.'),'.$decimal_prova_final.') AS rf');
		}
		else {
			$query->select('ROUND(('.$querySomaFinal.'),'.$decimal_prova_final.') AS rf');
		}
	...
	function getRanking( $options = array() )
	{
		...
		if(count($options['numero_etapas_final']) > 0):																																		//----
			$queryEtapaFinal = $this->_db->getQuery(true);																																	//----
			$queryEtapaFinal->select('COUNT(ResultsFinal.RF) AS TotalF');																													//----
			$queryEtapaFinal->select('GROUP_CONCAT(ResultsFinal.RF) AS RFinal');																											//----
			$queryEtapaFinal->select('GROUP_CONCAT(id_etapa) as id_etapaF');																												//----
			$queryEtapaFinal->select('GROUP_CONCAT(#__ranking_etapa.ordering) as ordering_etapaF');																							//----
			$queryEtapaFinal->select('GROUP_CONCAT(name_etapa SEPARATOR \'@@**@**@@\') as name_etapaF');																					//----
			$queryEtapaFinal->select('GROUP_CONCAT(id_local) as id_localF');																												//----
			$queryEtapaFinal->select('GROUP_CONCAT(name SEPARATOR \'@@**@**@@\') AS name_localF');																							//----																			//----
			$queryEtapaFinal->select('GROUP_CONCAT(ResultsFinal.special_status_etapa) AS special_status_etapa');																			//----
		...
		if(count($options['numero_etapas_final']) > 0) {																																	//----
			for ($i = 1; $i <= count($options['numero_etapas_final']); $i++) { 																												//----
				if($RFF != '')																																								//----
					$RFF .= '+';																																							//----
				$RNF = 	'IF(ResultadoFinal.TotalF > '.($i-1)																																//----
						. ', SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( ResultadoFinal.RFinal ORDER BY ResultadoFinal.ordering_etapaF ASC ),'											//----
						. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), 0)';																							//----
																																															//----
				$MULT =	'IF(ResultadoFinal.TotalF > '.($i-1)																																//----
						. ', SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( ResultadoFinal.special_value_etapa ORDER BY ResultadoFinal.ordering_etapaF ASC ),'								//----
						. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), 0)';																							//----
																																															//----
				$RFF .= 	'(('. $RNF .') * ('. $MULT .'))';																																//----
				$queryPosition->select( $RNF . ' AS RankFinal' . $i);																														//----
			}																																												//----
		}																																													//----
		else																																												//----
			$queryPosition->select( 'NULL as RankFinal' );                                                                                                                                	//----
		...
		$queryPosition->select( 'SUM(ResultadoPosition.rf) as rAddColsBefore1' );																											//----
																																															//----																																														
		if(count($options['numero_etapas_final']) > 0)																																		//----
			$queryPosition->select('ROUND((('.$RF .'))' .																																	//----
							'+('.$RFF.'),'.$decimal_prova_final.') AS RankRf');																												//----
		else																																												//----
			$queryPosition->select('ROUND((('.$RF .')/('.$options['results_prova'].')),'.$decimal_prova_final.') AS RankRf');																//----
																																															//----
																																															//----
		...
		$querySomaResultados = '';
		for ($i = 1; $i <= $options['results_prova']; $i++) { 
			if($querySomaResultados != '')	
				$querySomaResultados .= '+';	
				
			$querySomaResultados .= 'IF(COUNT(Resultados.rf) > '.($i-1)
								. ', SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( Resultados.rf ORDER BY Resultados.rf DESC ), '
								. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), 0)';	
		...
		$query->select('ROUND (('.$querySomaResultados.'),'.$options['decimal_prova'].') as rAddColsBefore1' );
		...
		if(count($options['numero_etapas_final']) > 0) {
			...
			for ($i = 1; $i <=count($options['numero_etapas_final']); $i++) { 
				...
				$queryResultadoFinal = 	'IF(Final.TotalF > '.($i-1)					
						. ', SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( Final.RFinal ORDER BY Final.ordering_etapaF ASC ),'
						. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), 0)';

				$queryMultiplicadorFinal =	'IF(Final.TotalF > '.($i-1)
						. ', SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( Final.special_value_etapa ORDER BY Final.ordering_etapaF ASC ),'
						. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), 0)';
				
				$querySomaFinais .= 	'(('. $queryResultadoFinal .') * ('. $queryMultiplicadorFinal .'))';

				$query->select( $queryResultadoFinal . ' AS rAddColsFinal' . $i);
								
				$query->select( 'IF(Final.TotalF > '.($i-1)
									. ', SUBSTRING_INDEX( SUBSTRING_INDEX( Final.id_etapaF, '
									. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), NULL) AS eAddColsFinal'.$i);	
			...
			$query->select('ROUND((('.$querySomaResultados.'))+('.$querySomaFinais.'),'.$decimal_prova_final.') AS rf');
		}
		else {
			$query->select('ROUND(('.$querySomaResultados.'),'.$decimal_prova_final.') AS rf');
		}
