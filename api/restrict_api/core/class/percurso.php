...
class TorneiosClassesPercurso {
	...
	const addColsBeforeClass = 0;
	const addColsAfterClass = 1;
	...
	{   
		...
		$document->addStyleSheet( 'administrator/components/com_torneios/classes/css/percurso.css?v=' . $token );
		$document->addScript( 'administrator/components/com_torneios/classes/js/percurso.js?v=' . $token  );		
		...
		$extension_custon = $extension . '_percurso';
	...
	//	$options['horarios'] = explode(',', $params_agenda_prova['horarios']);
		
	//	$options['inscricao']->id_prova;
		
		
		
		$query = $this->_db->getQuery(true);
		$query->select( $this->_db->quoteName( array('params_agenda_prova')));
		$query->from( $this->_db->quoteName('#__ranking_prova') );
		$query->where($this->_db->quoteName('id_prova') . '=' . $this->_db->quote($options['inscricao']->id_prova));
		$this->_db->setQuery($query);
		$params_agenda_prova = $this->_db->loadResult();
		if(!empty($params_agenda_prova)):
			$registry = new JRegistry;
			$registry->loadString($params_agenda_prova);
			$params_agenda_prova = $registry->toArray();
			$options['sequencias'] = explode(',', $params_agenda_prova['sequencia']);
			$options['horarios'] = explode(',', $params_agenda_prova['horarios']);			
		endif;

/*
		foreach($options['agendamento'] as $agendamento):
			$html .= '<span style="color:#ff0000;">'.JText::_('A Bateria número ').$agendamento->bateria_inscricao_agenda.JText::_(' inicia as ').$options['horarios'][$agendamento->bateria_inscricao_agenda].'</span>';
			$html .= '<div class="clearfix"></div>';
			$agendamento_text = $agendamento->turma_inscricao_agenda + (ceil($agendamento->turma_inscricao_agenda/8)-1)*2;
			$html .= '<span style="color:#ff0000;">'.JText::_('Seu Posto de inínio nesta bateria é o de número ').substr($agendamento_text,-1).'</span>';
			$html .= '<div class="clearfix"></div>';
		endforeach;
		$html .= '<span style="color:#ff0000;"><em>' . JText::_('O Atleta deve estar pronto no posto 30 minutos antes do início da bateria!') . '</em></span>';
*/
/*
		foreach($options['agendamento'] as $agendamento):
			$html .= '<span>'.JText::_('A Bateria de número ').'</span><span style="color:#ff0000;"><strong>'.$agendamento->bateria_inscricao_agenda.'</strong></span><span>'.JText::_(' inicia as ').'</span><span style="color:#ff0000;"><strong>'.$options['horarios'][$agendamento->bateria_inscricao_agenda].'hs</strong></span>.';
			$html .= '<div class="clearfix"></div>';
			$agendamento_text = $agendamento->turma_inscricao_agenda + (ceil($agendamento->turma_inscricao_agenda/8)-1)*2;
			$html .= '<span>'.JText::_('Seu Posto de inínio nesta bateria é o de número ').'</span><span style="color:#ff0000;"><strong>'.substr($agendamento_text,-1).'</strong></span>.';
			$html .= '<div class="clearfix"></div>';
		endforeach;
		$html .= '<span style="color:#ff0000;"><em>' . JText::_('O Atleta deve estar pronto no posto 30 minutos antes do início da bateria!') . '</em></span>';
...
*/
		...
		foreach($options['agendamento'] as $agendamento):
			$html .= '<span>'.JText::_('A Bateria de número ').'<strong>'.$agendamento->bateria_inscricao_agenda.'</strong>'.JText::_(' inicia às ').'<strong>'.$options['horarios'][$agendamento->bateria_inscricao_agenda-1].' hs</strong></span>.';
			$html .= '<div class="clearfix" style="padding-bottom:30px;"></div>';
			$agendamento_text = $agendamento->turma_inscricao_agenda + (ceil($agendamento->turma_inscricao_agenda/8)-1)*2;
		
			$html .= '<span style="color:#ff0000;font-size:16px;" ><strong>'.JText::_('Seu Posto de início nesta bateria é o de número ').substr($agendamento_text,-1).'</strong></span>.';
			$html .= '<div class="clearfix"></div>';
		endforeach;
		/*
		$html .= '<span style="color:#ff0000;"><em>' . JText::_('O Atleta deve estar pronto no posto 15 minutos antes do início da bateria!') . '</em></span>';
		$html .= '<div class="clearfix"></div>';
		$html .= '<span style="color:#ff0000;"><em>' . JText::_('Em caso de atraso, o atleta será remanejado para a última bateria, quando ainda houver vaga.') . '</em></span>';
	*/
	//	$html .= '<span style="color:#ff0000;"><em>' . JText::_('Nesta etapa, o número da posição não faz referência a nenhuma ordem ou sequência de aletas.') . '</em></span>';
	//	$html .= '<div class="clearfix"></div>';
	//	$html .= '<span style="color:#ff0000;"><em>' . JText::_('Os SQUADS serão montados pela organização da prova, tendo obrigatóriamente 01 atirador experiente responsável pelo mesmo.') . '</em></span>';
	//	$html .= '<div class="clearfix"></div>';
	//	$html .= '<span style="color:#ff0000;"><em>' . JText::_('A composição e ordem dos squads será disponibilizada pela FGCT, no site, no Sábado dia 17/10/20') . '</em></span>';
	...
	{		
		...
		$query->select( 'IF(ISNULL(turma_inscricao_agenda), \'-\',CONCAT(\'P\', RIGHT((turma_inscricao_agenda + (CEILING(turma_inscricao_agenda/8)-1)*2),1))) AS rAddColsAfter2');
		...
		$query->leftJoin( '#__ranking_inscricao_agenda USING(id_inscricao_etapa, id_prova, id_etapa )' );
	...
	{		
		...
		$queryRankPos->select( 'id_equipe');																																				//----
																																															//----
		for ($i = 1; $i <= $options['customize_value_prova']; $i++) { // AQUIIIIIII																															//----
			if(!empty($queryFinal))																																							//----
				$queryFinal .= ' + ';																																						//----
																																															//----
			$queryFinal .= 	'IF(COUNT(ResultadoRankPos.RF) > '.($i-1).																														//----
		...
		for ($i = 1; $i <= $options['customize_value_prova']; $i++) { 
			$query->select( 'IF(COUNT(ResultsEtapa.RF) > '.($i-1).
							', SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( ResultsEtapa.RF ORDER BY ResultsEtapa.RF DESC ),'
							.$this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), NULL) AS r' . $i);	
	...
	function getGenCatClass( $options = array())
	{
		...
		if( !$infoClass ):
			 $valor_atleta_classe = 0.00001;
		else:
			//if( $infoClass->vezes_atleta_prova < 6 && $infoClass->valor_atleta_classe >= 95):
		...
		$query->order('ponto_min_classe DESC');
		$this->_db->setQuery($query);
		$class = $this->_db->loadObject();
		
		if(empty($class->id_classe) && $infoUser->pcd_pf == '1'):
			$query->clear();
			$query->select( $this->_db->quoteName(array('id_classe',
														'name_classe',
														'id_categoria',
