...
class TorneiosClassesTiroabala0 {

	const addColsBeforeProva = 3;
	...
	{   
		...
		$document->addStyleSheet( 'administrator/components/com_torneios/classes/css/tiroabala0.css?v=' . $token  );	
		...
		$extension_custon = $extension . '_tiroabala0';
	...
	function getTypeProva()
	{
		...
		$object = new stdClass();
		$object->value = 1;
		$object->text = JText::_('TORNEIOS_VIEWS_PROVA_EDIT_TIROABALA0_TYPE_1');
		$returnTypeProva[] = $object;	
		
		$object = new stdClass();	
		$object->value = 2;
		$object->text = JText::_('TORNEIOS_VIEWS_PROVA_EDIT_TIROABALA0_TYPE_2');
		$returnTypeProva[] = $object;
		
		$object = new stdClass();	
		$object->value = 3;
		$object->text = JText::_('TORNEIOS_VIEWS_PROVA_EDIT_TIROABALA0_TYPE_3');
		$returnTypeProva[] = $object;
		
		$object = new stdClass();	
		$object->value = 4;
		$object->text = JText::_('TORNEIOS_VIEWS_PROVA_EDIT_TIROABALA0_TYPE_4');
		$returnTypeProva[] = $object;	
	...
	function getRelatory( $options = array() )
	{
		...
		SELECT  SUM(Inscricoes)  AS total_inscricoes, COUNT(DISTINCT( tcgf_ranking_inscricao.id_user ) ) AS total_atletas
		FROM (
			SELECT COUNT(DISTINCT(SIRES.nr_etapa_prova)) AS Inscricoes, SIRES.id_inscricao
			FROM tcgf_ranking_resultado SIRES
			INNER JOIN tcgf_ranking_inscricao SIINS USING(id_inscricao)
			INNER JOIN tcgf_ranking_inscricao_etapa SIIET USING(id_inscricao, id_etapa)
			INNER JOIN tcgf_ranking_campeonato SICAM USING(id_campeonato)
			INNER JOIN tcgf_ranking_modalidade SIMOD USING(id_modalidade)
			INNER JOIN tcgf_ranking_prova SIPRO USING(id_prova, id_campeonato)
			INNER JOIN tcgf_ranking_etapa SIETA USING(id_etapa, id_campeonato)
			INNER JOIN tcgf_users SIUSE ON( SIUSE.id = SIINS.id_user)
			WHERE SIRES.status_resultado = 1
			AND SIINS.status_inscricao = 1
			AND SIIET.status_inscricao_etapa = 1
			AND SIETA.status_etapa = 1
			AND SICAM.status_campeonato = 1
			AND SIPRO.status_prova = 1
			AND SIMOD.status_modalidade = 1
			AND (block =0 OR ano_campeonato < YEAR( CURDATE() ) )
			AND SICAM.id_campeonato IN ( ******* AQUI VAI AS IDS DOS COMPEONATOS *******) 
			GROUP BY id_inscricao,  SIRES.id_etapa, SIRES.nr_etapa_prova 
			) T1
		INNER JOIN tcgf_ranking_inscricao USING( id_inscricao ) 
		...
		*/
		...
		if ( (boolean) $relatory = $this->_db->loadObject() ) {
			$resp = '<p class="relatory">' . JText::sprintf('TORNEIOS_RELATORY_ATLETAS_TIROABALA0', $relatory->total_atletas) . '</p>';
            $resp .= '<p class="relatory">' . JText::sprintf('TORNEIOS_RELATORY_INCRICOES_TIROABALA0', $relatory->total_inscricoes) . '</p>';
		}
	...
	{	
		...
		//	$queryPosition->order( 'RankRS' . $i . ' DESC');																																//----
		//}																																													//----
																																															//----
		...
		$queryRank->select( 'GROUP_CONCAT(ResultadoRank.RankRf ORDER BY ' . $orderInit . $order . ' ) AS Rank' );																			//----
																																															//----
		...
		if($options['shot_off_prova']==1)
			...
			$query->select( 'FIND_IN_SET( Resultados.rf, (Position.Rank) ) AS rank');	
		...
		$query->innerJoin( $this->_db->quoteName('#__intranet_cidade') . ' USING('. $this->_db->quoteName('id_cidade').')' );	
	...
	{		
		...
		$queryResultsRF->select('id_etapa');																																				//----		
																																															//----
		$queryResultsRF->from( '(' . $queryResultsRS . ') ResultsRS' );																														//----
		$queryResultsRF->innerJoin( '#__ranking_inscricao USING( id_inscricao , id_equipe)' );																								//----
	...
	{		
		...
		$queryRankGeral->where( $this->_db->quoteName('Pagamento.baixa_pagamento').'<='. $this->_db->quoteName( 'RANETAPA.data_end_etapa' ) );												//----	
																																															//----
		$queryRankGeral->where('#__ranking_inscricao.id_equipe <> 7617');																													//----																																														
																																															//----
		if(!empty($options['id_etapa']))																																					//----							
		...
		$query->innerJoin( '#__ranking_campeonato USING( id_campeonato )' );
		$query->innerJoin( '#__ranking_etapa USING( id_etapa, id_campeonato )' );
		$query->innerJoin( '(' . $queryPagamento . ') PagamentoFim USING('. $this->_db->quoteName('id_user').','. $this->_db->quoteName('ano_campeonato').')');
		$query->where( $this->_db->quoteName('PagamentoFim.baixa_pagamento').'<='. $this->_db->quoteName( 'data_end_etapa' ) );																																														
												
		$query->innerJoin( '#__ranking_inscricao_etapa RANIETA USING( id_inscricao,  id_etapa )' );									
		$query->innerJoin( '#__users ON('. $this->_db->quoteName('id'). ' = '. $this->_db->quoteName('id_equipe'). ')' );																																	
																																														
		$query->where('#__ranking_inscricao.id_equipe <> 7617');
	...
	function getGeral( $options = array() )
	{
		...
		$options['nsocio_prova'] = 0;
		$options['convenio_prova'] = 1;
		$options['shot_off_prova'] = 0;
		$queryResultsRF = $this->getQueryRF( $options );
		...
		$queryRanking->select('#__ranking_etapa.ordering');																																	//----
																																															//----
		$queryRanking->from('('.$queryResultsRF.') AS ResultsRF' );																															//----
	...
	function getClassificacao( $options = array() )
	{
		...
		//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		$queryEtapaFinal = $this->_db->getQuery(true);																																		//----
		$queryEtapaFinal->select('ResultsFinal.RF AS RFinal');																																//----
		$queryEtapaFinal->select('id_etapa as id_etapaF');																																	//----
		$queryEtapaFinal->select('name_etapa as name_etapaF');																																//----
		$queryEtapaFinal->select('id_local as id_localF');																																	//----
		$queryEtapaFinal->select('name AS name_localF');																																	//----
		$queryEtapaFinal->select('id_inscricao');																																			//----
																																															//----
		$queryEtapaFinal->from('('.$queryResultsRF.') AS ResultsFinal' );																													//----
		$queryEtapaFinal->innerJoin( '#__ranking_inscricao USING( id_inscricao )' );																										//----
		$queryEtapaFinal->innerJoin( '#__ranking_campeonato USING( id_campeonato )' );																										//----
		$queryEtapaFinal->innerJoin( '#__ranking_inscricao_etapa USING( id_inscricao, id_etapa )' );																						//----
		$queryEtapaFinal->innerJoin( '#__ranking_etapa USING( id_etapa )' );																												//----
		$queryEtapaFinal->innerJoin( '#__users ON( id = id_local )' );																														//----
		$queryEtapaFinal->where('ResultsFinal.special_status_etapa = ' . $this->_db->quote( '1' ));																							//----
																																															//----
		$queryEtapaFinal->group('id_inscricao');																																			//----
		$queryEtapaFinal->group('id_etapa');																																				//----
																																															//----
		//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
																																															//----
		$queryRanking = $this->_db->getQuery(true);																																			//----
		...
		$queryRanking->innerJoin( '#__users ON( id = id_local )' );																															//----
																																															//----
		$queryRanking->where('ResultsRF.special_status_etapa = ' . $this->_db->quote( '0' ));																								//----
																																															//----
		if(!empty($options['id_prova']))																																					//----
			$queryRanking->where('id_prova = ' . $this->_db->quote( $options['id_prova'] ));																								//----
																																															//----
		...
		$queryPosition->select( 'IF(ISNULL(ResultadoFinal.RFinal), NULL, ResultadoFinal.RFinal) as RankFinal' );																			//----
                                                                                                                                                                                            //----
        if($options['decimal_prova'] == 0)																									                                             	//----
            $decimal_prova_final = 2;																														                                //----
        else                                                                                                                                                                                //----
            $decimal_prova_final = $options['decimal_prova'];																										                        //----
                                                                                                                                                                                            //----   
																																															
		$provasx3 = array(388, 389, 390, 391, 392, 393, 394, 395, ); // rifle internacional
		$provasx3 = array(468, 469, 470, 471, 472, 473, 474, 475, ); // rifle internacional 2022
		$provasx3 = array_merge($provasx3, array(400, 401, 402)); // WRABF
		$provasx3 = array_merge($provasx3, array(485, 486, 487)); // WRABF 2022
		$provasx3 = array_merge($provasx3, array(404, 405, 406, 407, 408)); // Field Target
		$provasx3 = array_merge($provasx3, array(436, 437, 438, 439, 440,441)); // Field Target
		$provasx3 = array_merge($provasx3, array(370, 371, 372, 373, 374, 375, 376, 383, 384,385, 386, 387)); // Nacionais Duelos
		$provasx3 = array_merge($provasx3, array(493, 494, 495, 496, 497, 498, 499, 500, 501,502, 503, 504)); // Nacionais Duelos 22
		$provasx3 = array_merge($provasx3, array(365, 366, 367, 368, 369)); // Nacionais Carabinas
		$provasx3 = array_merge($provasx3, array(488, 489, 490, 491, 492, 505, 506)); // Nacionais Carabinas 2022
		$provasx3 = array_merge($provasx3, array(417, 418, 419, 420, 421, 422, 423, 424)); // Duelos Indoor
		$provasx3 = array_merge($provasx3, array(442, 443, 444, 445, 446, 449, 466, 467)); // Duelos Indoor 2022
		$provasx3 = array_merge($provasx3, array(425, 426, 427, 428, 429, 430, 431,432, 433, 434)); // ISSF
		//$provasx3 = array_merge($provasx3, array(476, 477, 478, 479, 480, 481, 482, 483, 84)); // ISSF 2022
		$provasx3 = array_merge($provasx3, array(362, 363, 396, 397, 398, 399)); // Ar Comprimido
		$provasx3 = array_merge($provasx3, array(460, 461, 462, 463, 464, 465)); // Ar Comprimido
		if(in_array($options['id_prova'], $provasx3))
        	$queryPosition->select('ROUND((('.$RF .')/('.$options['results_prova'].'))' .																									//----
							   '+(IF(ISNULL(ResultadoFinal.RFinal), 0, (ResultadoFinal.RFinal)*3)),'.$decimal_prova_final.') AS RankRf');							                        //----	
		//elseif(in_array($options['id_prova'], $provasx5))
        //	$queryPosition->select('ROUND((('.$RF .')/('.$options['results_prova'].'))' .																									//----
		//					   '+(IF(ISNULL(ResultadoFinal.RFinal), 0, (ResultadoFinal.RFinal)*5)),'.$decimal_prova_final.') AS RankRf');							                        //----	
		else
        	$queryPosition->select('ROUND((('.$RF .')/('.$options['results_prova'].'))' .																									//----
							   '+(IF(ISNULL(ResultadoFinal.RFinal), 0, (ResultadoFinal.RFinal)*2)),'.$decimal_prova_final.') AS RankRf');							                        //----	
																																															//----
		...
		$queryPosition->leftJoin('('.$queryEtapaFinal.') ResultadoFinal USING(id_inscricao)');																								//----
																																															//----
		$queryPosition->order($this->_db->quoteName('RankRf') . ' DESC');																													//----
																																															//----
		for ($i = 1; $i <=  $options['results_prova']; $i++) { 																																//----
		...
		for ($i = 1; $i <= $options['results_prova']; $i++) { 
			...
			$querySomaFinal .= 'IF(COUNT(Resultados.rf) > '.($i-1)
		...
		$query->select('ROUND (('.$querySomaFinal.'),'.$options['decimal_prova'].') as rAddColsBefore1' );
		...
		$query->select('ROUND (( ('.$querySomaFinal.') / (' . $options['results_prova'] . ') ),'.$decimal_prova_final.') as rAddColsBefore2');

		$query->select('IF(ISNULL(Final.RFinal), NULL, Final.RFinal) as rAddColsBefore3');
		$query->select('IF(ISNULL(Final.RFinal), NULL, Final.id_etapaF) as eAddColsBefore3');
		$query->select('IF(ISNULL(Final.RFinal), NULL, Final.name_etapaF) as enAddColsBefore3');
		$query->select('IF(ISNULL(Final.RFinal), NULL, Final.id_localF) as lAddColsBefore3');
		$query->select('IF(ISNULL(Final.RFinal), NULL, Final.name_localF) as lnAddColsBefore3');		
		...
		$provasx3 = array_merge($provasx3, array(460, 461, 462, 463, 464, 465)); // Ar Comprimido
		if(in_array($options['id_prova'], $provasx3))
			$query->select('ROUND((('.$querySomaFinal.')/('.$options['results_prova'].'))+(IF(ISNULL(Final.RFinal), 0, (Final.RFinal)*3)),'.$decimal_prova_final.') AS rf');
		//elseif(in_array($options['id_prova'], $provasx5))
		//	$query->select('ROUND((('.$querySomaFinal.')/('.$options['results_prova'].'))+(IF(ISNULL(Final.RFinal), 0, (Final.RFinal)*5)),'.$decimal_prova_final.') AS rf');	
		else
			$query->select('ROUND((('.$querySomaFinal.')/('.$options['results_prova'].'))+(IF(ISNULL(Final.RFinal), 0, (Final.RFinal)*2)),'.$decimal_prova_final.') AS rf');
		...
		$query->from( $this->_db->quoteName('#__ranking_inscricao') );
		$query->innerJoin('(' . $queryRanking . ') Resultados USING('. $this->_db->quoteName('id_inscricao').','. $this->_db->quoteName('id_classe').')' );	
		$query->leftJoin('('.$queryEtapaFinal.') Final USING( id_inscricao )');
	...
	function getSpecialEtapaView( $type, $special_status_etapa, $special_value_etapa, $special_text_etapa	)
	{
		...
		$returnView .= JText::_( 'TORNEIOS_VIEWS_ETAPA_EDIT_TIROABALA0_SPECIAL_ETAPA_STATUS'  );  
	...
	function getTitleTableSlide( $options )
	{
		...
		for ($i = self::addColsBeforeProva; $i >= 1; $i--) { 
			$title[] = '	<span style="width:10%; float:right; top: 5px; text-align:center">' . mb_strtoupper( JText::sprintf('TORNEIOS_VIEWS_RESULT_PROVA_NAME_TH_ADDCOLBEFORE_' . self::tagLanguage . $options['type_prova']  . '_' . $i, $i) ) . '</span>';		 
		}
	...
	function getResultSlide( $options = array() )
	{
		...
		for ($i = 1; $i <= $options['results_prova']; $i++) { 
			...
			$query->select( 'IF(COUNT(Resultados.value_resultados) > '.($i-1).',
								SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( Resultados.id_etapas ORDER BY Resultados.value_resultados DESC ),'.$this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1),
								NULL) AS e'.$i);	
										
			$query->select( 'IF(COUNT(Resultados.value_resultados) > '.($i-1).',
								SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( Resultados.name_etapas ORDER BY Resultados.value_resultados DESC SEPARATOR \'@@**@**@@\' ),'.$this->_db->quote('@@**@**@@').', '.($i).'),'.$this->_db->quote('@@**@**@@').', -1),
								NULL) AS en'.$i);	
								
			$query->select( 'IF(COUNT(Resultados.value_resultados) > '.($i-1).',
								SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( Resultados.id_locals ORDER BY Resultados.value_resultados DESC ),'.$this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1),
								NULL) AS l'.$i);	
										
			$query->select( 'IF(COUNT(Resultados.value_resultados) > '.($i-1).',
								SUBSTRING_INDEX( SUBSTRING_INDEX( GROUP_CONCAT( Resultados.names ORDER BY Resultados.value_resultados DESC SEPARATOR \'@@**@**@@\' ),'.$this->_db->quote('@@**@**@@').', '.$i.'),'.$this->_db->quote('@@**@**@@').', -1),
								NULL) AS ln'.$i);	
		}
		...
		if(!empty($querySoma))
			$query->select( 'ROUND (('.$querySoma.') , '. $decimalMedia .' ) as rAddColsBefore1');
		...
		if(!empty($queryFator))
			$query->select( 'ROUND (( ('.$queryFator.') / (' .$options['results_prova']. ') ), 2 ) as rAddColsBefore2');
		
		
		$query->select( 'IF(ISNULL(Final.value_resultados),\'&nbsp;\',Final.value_resultados) as rAddColsBefore3');
		
		
		
		//$query->select( 'IF(ISNULL(Final.value_resultados),NULL,Final.id_etapas) as eAddColsBefore3');
		////$query->select( 'IF(ISNULL(Final.value_resultados),NULL,Final.name_etapas) as enAddColsBefore3');
		//$query->select( 'IF(ISNULL(Final.value_resultados),NULL,Final.id_locals) as lAddColsBefore3');
		//$query->select( 'IF(ISNULL(Final.value_resultados),NULL,Final.names) as lnAddColsBefore3');	
		...
		if(!empty($queryFinal))
			$query->select( 'ROUND (( ('.$queryFinal.') / (' .$options['results_prova']. ') + ( IF(ISNULL(Final.value_resultados),0,Final.value_resultados * 2) )  ), 2 ) as rf');
		...
		$queryRankGroup->select( 'IF(ISNULL(FinalRank.value_resultados_final),NULL,FinalRank.value_resultados_final) as RankFinal');	
		...
		if(!empty($queryFinalRank))
			$queryRankGroup->select( 'ROUND (( ('.$queryFinalRank.') / (' .$options['results_prova']. ') + ( IF(ISNULL(FinalRank.value_resultados_final),0,FinalRank.value_resultados_final * 2) )  ), 2 ) as RankRf');
		...
		$queryRankGroup->innerJoin( $this->_db->quoteName('#__intranet_pf') . 'USING('. $this->_db->quoteName('id_user').')' );
		$queryRankGroup->innerJoin( $this->_db->quoteName('#__intranet_associado') . 'USING('. $this->_db->quoteName('id_user').')' );
		$queryRankGroup->innerJoin( $this->_db->quoteName('#__users') . 'ON('. $this->_db->quoteName('id'). ' = '. $this->_db->quoteName('id_user').')' );
		...
		$queryResultSerieRank->innerJoin( '#__ranking_inscricao_etapa ReRaInsEta USING( id_inscricao,  id_etapa )' );																//----
		...
		$queryResultSerieRank->where('ReRaInsEta.status_inscricao_etapa = 1');																										//----
		...
		$queryMelhorSerieRank = $this->_db->getQuery(true);																															//----
																																													//----																																//----
		$queryMelhorSerieRank->select( 'MAX(SerieRF) AS value_resultados_rank');																									//----
							 																																						//----
		...
		//-------------------------------------------------------------- RESULTADO SÉRIE ETAPAS CADA INSCRIÇÃO (RANKING) ----------------------------------------------------------------
																																													//----
		$queryResultSerieRankFinal = $this->_db->getQuery(true);																													//----
																																													//----
		switch( $options['rs_etapa_prova'] )																																		//----
		{																																											//----
			case '1':																																								//----
				$queryResultSerieRankFinal->select( 'MAX(ReRaResult.value_resultado) AS SerieRF');																					//----
			break;																																									//----
																																													//----
			case '2':																																								//----
				$RFString = '';																																						//----
				...
				if($options['decimal_prova']==0)																																	//----
					$options['decimal_prova'] = '2';																																//----
																																													//----
				$queryResultSerieRankFinal->select( 'ROUND ( ( (' . $RFString . ') / (' . $options['ns_etapa_prova'] . ')), '. $options['decimal_prova'] .' ) AS SerieRF');			//----
																																													//----
			break;																																									//----
																																													//----
			case '3':																																								//----
				...
				for ($i = 1; $i <= $options['ns_etapa_prova']; $i++) { 																												//----
					...
					$RFString = $RFString . ' MAX(IF(ReRaResult.ordering = '. $i .', ReRaResult.value_resultado, 0)) ';																//----
																																													...
																																													//----
				$queryResultSerieRankFinal->select( '( ' . $RFString . ' ) AS SerieRF');																							//----
			break;																																									//----
		}																																											//----
																																													//----
		$queryResultSerieRankFinal->select( 'id_inscricao');																														//----
		$queryResultSerieRankFinal->select( 'id_etapa');																															//----
																																													//----
		$queryResultSerieRankFinal->from( '#__ranking_resultado ReRaResult' );																										//----
		$queryResultSerieRankFinal->innerJoin( '#__ranking_inscricao ReRaInscri USING( id_inscricao )' );																			//----
		$queryResultSerieRankFinal->innerJoin( '#__ranking_inscricao_etapa ReRaInsEta USING( id_inscricao,  id_etapa )' );															//----
		$queryResultSerieRankFinal->innerJoin( '#__ranking_etapa ReRaEtapa USING( id_etapa )' );																					//----
																																													//----
		if($options['id_classe'])																																					//----
			$queryResultSerieRankFinal->where('ReRaInscri.id_classe = ' . $this->_db->quote( $options['id_classe'] ));																//----
																																													//----
		if($options['id_categoria'])																																				//----
			$queryResultSerieRankFinal->where('ReRaInscri.id_categoria = ' . $this->_db->quote( $options['id_categoria'] ));														//----
																																													//----
		if($options['id_genero'])																																					//----
			$queryResultSerieRankFinal->where('ReRaInscri.id_genero = ' . $this->_db->quote( $options['id_genero'] ));																//----
																																													//----
		if(!empty($options['id_prova']))																																			//----
			$queryResultSerieRankFinal->where('ReRaInscri.id_prova = ' . $this->_db->quote( $options['id_prova'] ));																//----
																																													//----
		$queryResultSerieRankFinal->where('ReRaResult.status_resultado = 1');																										//----
		$queryResultSerieRankFinal->where('ReRaInscri.status_inscricao = 1');																										//----
		$queryResultSerieRankFinal->where('ReRaInsEta.status_inscricao_etapa = 1');																									//----
		$queryResultSerieRankFinal->where('ReRaEtapa.status_etapa = 1');																											//----
		$queryResultSerieRankFinal->where('ReRaEtapa.state_etapa = 1');																												//----
		$queryResultSerieRankFinal->where('ReRaEtapa.special_status_etapa = 1');																									//----
																																													//----
																																													//----
		$queryResultSerieRankFinal->group('ReRaResult.id_inscricao');																												//----
		$queryResultSerieRankFinal->group('ReRaResult.id_etapa');																													//----
		$queryResultSerieRankFinal->group('ReRaResult.nr_etapa_prova');																												//----
																																													//----
		//---------------------------------------------------- RESULTADO CÁLCULO DOS VALORES DE CADA INSCRIÇÃO DA ETAPA (RANKING) --------------------------------------------------//----
																																													//----
		$queryResultadoFinalRank = $this->_db->getQuery(true);																														//----
																																													//----																																//----
		$queryResultadoFinalRank->select( 'MAX(SerieRF) AS value_resultados_final');																								//----
							 																																						//----
		$queryResultadoFinalRank->select( 'id_inscricao');						 																									//----
		$queryResultadoFinalRank->select( 'id_etapa' );																																//----
		$queryResultadoFinalRank->from( '(' . $queryResultSerieRankFinal . ') MelhorSerie' );			 																			//----
	 					 																																							//----
	 	$queryResultadoFinalRank->group('id_inscricao');						 																									//----
		$queryResultadoFinalRank->group('id_etapa');						 																										//----
						 																																							//----
		$queryRankGroup->leftJoin('(' . $queryResultadoFinalRank . ') FinalRank USING('. $this->_db->quoteName('id_inscricao').')' );												//----
		//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		$queryRank = $this->_db->getQuery(true);																				
		$queryRank->select( 'GROUP_CONCAT( ResultadoSoma.id_user )' );
		$queryRank->from( '(' . $queryRankGroup . ') ResultadoSoma' );		
		$query->select( 'FIND_IN_SET( ( id_user ), (' . $queryRank . ') ) AS rank');
		...
		$queryMelhorSerie = $this->_db->getQuery(true);																																//----
																																													//----																																//----
		$queryMelhorSerie->select( 'MAX(SerieRF) AS value_resultados');																												//----
							 																																						//----
		...
		//------------------------------------------------------------ RESULTADO SÉRIE ETAPAS CADA INSCRIÇÃO (ETAPA FINAL) ---------------------------------------------------------------
																																													//----
		$queryResultadoSerieFinal = $this->_db->getQuery(true);																														//----
																																													//----
		switch( $options['rs_etapa_prova'] )																																		//----
		{																																											//----
			case '1':																																								//----
				$queryResultadoSerieFinal->select( 'MAX(ReFiResult.value_resultado) AS SerieRF');																					//----
			break;																																									//----
																																													//----
			case '2':																																								//----
				$RFString = '';																																						//----
				for ($i = 1; $i <= $options['ns_etapa_prova']; $i++) { 																												//----
					if(!empty($RFString))																																			//----
						$RFString = $RFString . ' + ';																																//----
					$RFString = $RFString . ' MAX(IF(ReFiResult.ordering = '. $i .', ReFiResult.value_resultado, 0)) ';																//----
				}																																									//----
				if($options['decimal_prova']==0)																																	//----
					$options['decimal_prova'] = '2';																																//----
																																													//----
				$queryResultadoSerieFinal->select( 'ROUND ( ( (' . $RFString . ') / (' . $options['ns_etapa_prova'] . ')), '. $options['decimal_prova'] .' ) AS SerieRF');			//----
																																													//----
			break;																																									//----
																																													//----
			case '3':																																								//----
				...
				for ($i = 1; $i <= $options['ns_etapa_prova']; $i++) { 																												//----
					...
					$RFString = $RFString . ' MAX(IF(ReFiResult.ordering = '. $i .', ReFiResult.value_resultado, 0)) ';																//----
																																													...
																																													//----
				$queryResultadoSerieFinal->select( '( ' . $RFString . ' ) AS SerieRF');																								//----
			break;																																									//----
		}																																											//----
																																													//----
		$queryResultadoSerieFinal->select( 'ReFiResult.id_etapa AS id_etapas' );																									//----
		$queryResultadoSerieFinal->select( 'ReFiEtapa.name_etapa AS name_etapas' );																									//----
		$queryResultadoSerieFinal->select( 'ReFiInsEta.id_local AS id_locals' );																									//----
		$queryResultadoSerieFinal->select( 'ReFiUsers.name AS names' );																												//----
																																													//----
		$queryResultadoSerieFinal->select( 'id_inscricao');																															//----
		$queryResultadoSerieFinal->select( 'id_etapa');																																//----
																																													//----
		$queryResultadoSerieFinal->from( '#__ranking_resultado ReFiResult' );																										//----
		$queryResultadoSerieFinal->innerJoin( '#__ranking_inscricao ReFiInscri USING( id_inscricao )' );																			//----
		$queryResultadoSerieFinal->innerJoin( '#__ranking_inscricao_etapa ReFiInsEta USING( id_inscricao,  id_etapa )' );															//----
		$queryResultadoSerieFinal->innerJoin( '#__ranking_etapa ReFiEtapa USING( id_etapa )' );																						//----
		$queryResultadoSerieFinal->leftJoin( '#__users ReFiUsers ON (ReFiUsers.id = ReFiInsEta.id_local)' );																		//----
																																													//----
		if($options['id_classe'])																																					//----
			$queryResultadoSerieFinal->where('ReFiInscri.id_classe = ' . $this->_db->quote( $options['id_classe'] ));																//----
																																													//----
		if($options['id_categoria'])																																				//----
			$queryResultadoSerieFinal->where('ReFiInscri.id_categoria = ' . $this->_db->quote( $options['id_categoria'] ));															//----
																																													//----
		if($options['id_genero'])																																					//----
			$queryResultadoSerieFinal->where('ReFiInscri.id_genero = ' . $this->_db->quote( $options['id_genero'] ));																//----
																																													//----
		if(!empty($options['id_prova']))																																			//----
			$queryResultadoSerieFinal->where('ReFiInscri.id_prova = ' . $this->_db->quote( $options['id_prova'] ));																	//----
																																													//----
		$queryResultadoSerieFinal->where('ReFiResult.status_resultado = 1');																										//----
		$queryResultadoSerieFinal->where('ReFiInscri.status_inscricao = 1');																										//----
		$queryResultadoSerieFinal->where('ReFiInsEta.status_inscricao_etapa = 1');																									//----
		$queryResultadoSerieFinal->where('ReFiEtapa.status_etapa = 1');																												//----
		$queryResultadoSerieFinal->where('ReFiEtapa.state_etapa = 1');																												//----
		$queryResultadoSerieFinal->where('ReFiEtapa.special_status_etapa = 1');																										//----
																																													//----
																																													//----
		$queryResultadoSerieFinal->group('ReFiResult.id_inscricao');																												//----
		$queryResultadoSerieFinal->group('ReFiResult.id_etapa');																													//----
		$queryResultadoSerieFinal->group('ReFiResult.nr_etapa_prova');																												//----
																																													//----
		//--------------------------------------------------- RESULTADO CÁLCULO DOS VALORES DE CADA INSCRIÇÃO DA ETAPA (SEM FINAL) -------------------------------------------------//----
																																													//----
		$queryMelhorSerieFinal = $this->_db->getQuery(true);																														//----
																																													//----																																//----
		$queryMelhorSerieFinal->select( 'MAX(SerieRF) AS value_resultados');																										//----
							 																																						//----
		$queryMelhorSerieFinal->select( 'id_inscricao');						 																									//----
		$queryMelhorSerieFinal->select( 'id_etapas' );																																//----
		$queryMelhorSerieFinal->select( 'name_etapas' );																															//----
		$queryMelhorSerieFinal->select( 'id_locals' );																																//----
		$queryMelhorSerieFinal->select( 'names' );																			 														//----
		$queryMelhorSerieFinal->from( '(' . $queryResultadoSerieFinal . ') MelhorSerieFinal' );						 																//----
	 					 																																							//----
	 	$queryMelhorSerieFinal->group('id_inscricao');						 																										//----
		$queryMelhorSerieFinal->group('id_etapa');						 																											//----
						 																																							//----
		$query->leftJoin('(' . $queryMelhorSerieFinal . ') Final USING('. $this->_db->quoteName('id_inscricao').')' );																//----
		//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		{																																											//----
			case '1':																																								//----
				$queryResultadoEtapa->select( 'MAX(C.value_resultado) AS value_resultados' );																						//----
			break;																																									//----
																															 														//----
			case '2':																																								//----
				$queryResultadoEtapa->select( 'ROUND ( ( SUM(C.value_resultado) / (' . $options['ns_etapa_prova'] . ') ), '. $options['decimal_prova'] .' ) AS value_resultados' );	//----
			break;																																									//----
																															 														//----
			case '3':																																								//----
				$queryResultadoEtapa->select( 'SUM(C.value_resultado) AS value_resultados' );																						//----
			break;																																		 							//----
		}																																											//----
													 																																//----
		$queryResultadoEtapa->select( 'C.id_etapa AS id_etapas' );																													//----
		$queryResultadoEtapa->select( 'CCCC.name_etapa AS name_etapas' );																											//----
		$queryResultadoEtapa->select( 'CCC.id_local AS id_locals' );																												//----
		$queryResultadoEtapa->select( 'CCCCC.name AS names' );																														//----
		$queryResultadoEtapa->from( '#__ranking_resultado C' );																														//----
		$queryResultadoEtapa->innerJoin( '#__ranking_inscricao CC USING(id_inscricao)' );																							//----
		$queryResultadoEtapa->innerJoin( '#__ranking_inscricao_etapa CCC USING(id_inscricao, id_etapa)' );																			//----
		$queryResultadoEtapa->innerJoin( '#__ranking_etapa CCCC USING (id_etapa)' );																								//----
		$queryResultadoEtapa->leftJoin( '#__users CCCCC ON (CCCCC.id = CCC.id_local)' );																							//----
																																													//----
		if($options['id_classe'])																																					//----
		...
		$queryResultadoEtapa->where('CCC.status_inscricao_etapa = 1');																												//----
		...
		$queryResultadoEtapa->group('C.id_inscricao');																																//----
																																													//----
		$query->innerJoin('(' . $queryResultadoEtapa . ') Etapa USING('. $this->_db->quoteName('id_inscricao').')' );																//----
		//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		$query->order($this->_db->quoteName('rf') . ' DESC');
		$query->order($this->_db->quoteName('rAddColsBefore3') . ' DESC');
	...
	function getTitleTableSlideEtapa( $options )
	{
		...
		//for ($i = self::addColsEtapa; $i >= 1; $i--) { 
		//	$title[] = '	<span style="width:10%; float:right; top: 5px; text-align:center">' .mb_strtoupper( JText::sprintf('TORNEIOS_VIEWS_RESULT_ETAPA_NAME_ADDCOLS_TH_' . self::tagLanguage . $options['type_prova'] , $i) ) . '</span>';		 
		//}
		
		$title[] = '<span style="width:10%; float:right; top: 5px; text-align:center">' . mb_strtoupper( JText::_( 'TORNEIOS_VIEWS_RESULT_ETAPA_NAME_' . self::tagLanguage .'RF' ) ) . '</span>';
		
		
		
		for ($i = $options['nr_etapa_prova']; $i >= 1; $i--) { 
			for ($x = $options['ns_etapa_prova']; $x >= 1; $x--) {
				if ( $options['ns_etapa_prova'] > 1 ): 
					$nrName = $x;
				else:
					$nrName = $i;
				endif;	
				
				if ( $item->special_names_prova ): 
					$title[] = '	<span style="width:6%; float:right; top: 5px; text-align:center">' .mb_strtoupper( JText::_('TORNEIOS_VIEWS_RESULT_ETAPA_NAME_TH_' . self::tagLanguage . $options['type_prova'] . '_' . $nrName) ) . '</span>';
				else:		 
					$title[] = '	<span style="width:6%; float:right; top: 5px; text-align:center">' .mb_strtoupper( JText::sprintf('TORNEIOS_VIEWS_RESULT_ETAPA_NAME_TH_' . self::tagLanguage . $options['type_prova'] , $nrName) ) . '</span>';
				endif; 
				//$title[] = '	<span style="width:10%; float:right; top: 5px; text-align:center">' .mb_strtoupper( JText::sprintf('TORNEIOS_VIEWS_RESULT_ETAPA_NAME_TH_' . self::tagLanguage . $options['type_prova'] . '_' . $i, $i) ) . '</span>';		 
...
*/
	...
	{		
		...
		for ($i = 1; $i <= $options['nr_etapa_prova']; $i++) { 
			for ($x = 1; $x <= $options['ns_etapa_prova']; $x++) { 
				$query->select( 'MAX( IF(C.nr_etapa_prova = ' . $i . ' AND C.ordering = ' . $x . ', C.value_resultado, \'&nbsp;\')) AS r' . $i . 's' . $x);
			}
		}
		
		if($options['nr_etapa_prova']>1 && $options['ns_etapa_prova']>1) :
			for ($i = 1; $i <= $options['nr_etapa_prova']; $i++) { 
			 $query->select( 'MAX(IF(ResultadoSerie.id_inscricao = C.id_inscricao AND ResultadoSerie.nr_etapa_prova = '. $i .', ResultadoSerie.RSerie, \'&nbsp;\')) AS rs' . $i);
			}
		endif;
		...
		$queryRankResults = $this->_db->getQuery(true);																																		//----
																																															//----
		$queryRankResults->select( 'RANASS.id_associado AS RankMatricula');																													//----
																																															//----
		for ($i = 1; $i <= $options['ns_etapa_prova']; $i++) {  																															//----
		...
		$queryRankResults->innerJoin( '#__ranking_resultado RANRESU USING( id_inscricao )' );																								//----				
																																															//----								
		$queryRankResults->innerJoin( '#__intranet_associado RANASS USING('. $this->_db->quoteName('id_user').')' );																		//----
		...
		{																																													//----
			...
			case '4':																																										//----
				...
				$query->select( 'FIND_IN_SET( UUUUU.id_associado, (' . $queryRank . ') ) AS rank');																							//----
			break;																																											//----
																																															//----
			case '2':																																										//----
			case '3':																																										//----
		...
		$query->innerJoin( '#__intranet_associado UUUUU USING('. $this->_db->quoteName('id_user').')' );
		...
		$queryRF->innerJoin( '#__ranking_resultado RFRES USING( id_inscricao )' );																								//----
																																												//----
		$queryRF->innerJoin( '#__intranet_associado RFASS USING('. $this->_db->quoteName('id_user').')' );																		//----
		...
		$queryOrderResult = $this->_db->getQuery(true);																																		//----
																																															//----
		$queryOrderResult->select( 'ORDASS.id_associado AS OrderMatricula');																												//----
																																															//----
		for ($i = 1; $i <= $options['ns_etapa_prova']; $i++) {  																															//----
		...
		$queryOrderResult->innerJoin( '#__ranking_resultado ORDRESU USING( id_inscricao )' );																								//----
																																															//----							
		$queryOrderResult->innerJoin( '#__intranet_associado ORDASS USING('. $this->_db->quoteName('id_user').')' );																		//----
		$queryOrderResult->innerJoin( '#__users ORDUSER ON('. $this->_db->quoteName('id'). ' = '. $this->_db->quoteName('id_user').')' );													//----
																																															//----
		$queryOrderResult->where( '(' 																																						//----
									. '(' 																																					//----
										. $this->_db->quoteName('status_associado') . '=' . $this->_db->quote( '1' ) 																		//----
										. ' AND '																																			//----
										. $this->_db->quoteName('validate_associado') . '>=' . $this->_db->quote( JFactory::getDate('now', $this->_siteOffset)->format('Y-m-d',true) ) 		//----			
		...
		$query->order($this->_db->quoteName('UUUUU.id_associado') . ' ASC');
	...
	{		
		...
		$queryPartEtapas->select( 'id_equipe');																																				//----
		...
		$queryPartEtapas->where( $this->_db->quoteName('Pagamento.baixa_pagamento').'<='. $this->_db->quoteName( 'ETAPAORDER.data_end_etapa' ) );											//----																																														
																																															//----
		$queryPartEtapas->where('#__ranking_inscricao.id_equipe <> 7617');																													//----
																																															//----																																														
		if(!empty($options['id_prova']))																																					//----
