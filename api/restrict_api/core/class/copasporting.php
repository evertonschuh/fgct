...
class TorneiosClassesCopaSporting {
	...
	const tagLanguage =  'COPASPORTING_';
	...
	{   
		...
		$document->addStyleSheet( 'administrator/components/com_torneios/classes/css/copasporting.css?v=' . $token  );	
		$document->addScript( 'administrator/components/com_torneios/classes/js/copasporting.js?v=' . $token  );	
		...
		$extension_custon = $extension . '_copasporting';
...
*/
		...
		foreach($options['agendamento'] as $agendamento):
			...
			$html .= '<div class="clearfix"></div>';
			$agendamento_text = $agendamento->turma_inscricao_agenda + (ceil($agendamento->turma_inscricao_agenda/8)-1)*2;
			$html .= '<span>'.JText::_('Seu Posto de início nesta bateria é o de número ').'<strong>'.substr($agendamento_text,-1).'</strong></span>.';
			$html .= '<div class="clearfix"></div>';
		endforeach;
	...
	{		
		...
		$query->select( 'IF(ISNULL(name_cidade),\'Não Cadastrado\', name_cidade) as name_cidade');
	...
	function getRanking( $options = array() )
	{
		...
		if($options['numero_etapas'] > $options['results_prova'])
		for ($i = ($options['results_prova'] + 1); $i <= $options['numero_etapas']; $i++) { 
