...
class TorneiosClassesCopaBrasil {
	...
	{   
		...
		$document->addStyleSheet( 'administrator/components/com_torneios/classes/css/copabrasil.css?v=' . $token  );		
		...
		$extension_custon = $extension . '_copabrasil';
	...
	function getClassificacao( $options = array() )
	{
		...
		$RN = '';																																											//----
																																															//----
		for ($i = 1; $i <= $options['numero_etapas']; $i++) { 																																//----
			if(!empty($RF) && $i <= $options['results_prova'])																																//----
				$RF .= ' + ';																																								//----
																																															//----
			$RN = 	'IF(COUNT(ResultadoPosition.rf) > '.($i-1)																																//----
					...
					. $this->_db->quote(',').', '.$i.'),'.$this->_db->quote(',').', -1), 0)';																								//----
																																															//----
			if($i <= $options['results_prova'])																																				//----
				$RF .= $RN;																																									//----	
																																															//----
			$queryPosition->select( 'ROUND (('. $RN . '), 3 ) AS RankR' . $i);																												//----
		}																																													//----
																																															//----
		if(!empty($RF))																																										//----
		...
		for ($i = 1; $i <= $options['numero_etapas']; $i++) { 
			...
			if(!empty($queryFinal) && $i <= $options['results_prova'])	
				$queryFinal .= ' + ';
			...
			$query->select( 'ROUND (('. $querySoma . '), 3 ) AS r' . $i);		
