...
class TorneiosClassesCompak {
	...
	const specialEtapa = false;
	const multiLocal = false;
	...
	{   
		...
		$extension_custon = $extension . '_compak';
 ...
 /*
		...
		$html .= '<label class="col-sm-3 control-label">' . JText::_('Reservas:') . '</label>';
		$html .= '<div class="col-sm-9">';
		$html .= '<div class="row">';
		$html .= '<div class="col-sm-12" >';
		
		$html .= '<div class="checkbox checkbox-success">';
		$html .= '<input id="almoco_' .  $options['id_prova'] . '" type="checkbox" class="check-part" '.(in_array('almoco',$params_inscricao_etapa) ? 'checked="checked "' : '').'data-prova="'.$options['id_prova'].'" name="params_inscricao_etapa['.$options['id_prova'].'][]" value="almoco">';
		$html .= '<label for="almoco_' .  $options['id_prova'] . '">' . JText::_('Marque para reservar a participação no almoço'). '</label>';
		$html .= '</div>';
		
		$html .= '<div class="checkbox checkbox-success">';
		$html .= '<input id="insumos_' .  $options['id_prova'] . '" type="checkbox" class="check-part" '.(in_array('insumos',$params_inscricao_etapa) ? 'checked="checked "' : '').'data-prova="'.$options['id_prova'].'" name="params_inscricao_etapa['.$options['id_prova'].'][]" value="insumos">';
		$html .= '<label for="insumos_' .  $options['id_prova'] . '">' . JText::_('Marque para reservar insumos'). '</label>';
		$html .= '</div>';
	...
	function getRanking( $options = array() )
	{
		...
		if(isset($options['id_user']) && $options['id_user'])
			$query->where($this->_db->quoteName('AtletaDados.id') . ' = ' . $this->_db->quote( $options['id_user'] ));					
	...
	function getResultsEtapaQuery( $options = array() )
	{		
		//----------------------------------------------------------------- RESULTADO SÉRIE ETAPAS CADA INSCRIÇÃO(SEM FINAL) ---------------------------------------------------------------------
		$queryResultsRS = $this->_db->getQuery(true);																																		//----
																																															//----
