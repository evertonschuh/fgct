...
class EASistemasModelSignatures extends JModelList
{
	var $_db = null;
	var $_user = null;
	var $_app = null;	
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        $this->context .'list.ordering','filter_order', 'id_signature', 'string');
    ...
    protected function getListQuery() {
		...
		$query->select($this->_db->quoteName(  array('id_signature',
													 'status_signature',
													 'name_signature',
													 'checked_out',
													 'checked_out_time',
        ...
        if ($status!='')
			$query->where($this->_db->quoteName('status_signature') . '=' . $this->_db->quote($this->_db->escape($status)));
		else
			$query->where($this->_db->quoteName('status_signature') . '>=' . $this->_db->quote($this->_db->escape('0')));
				 
		$search = $this->getState('filter.search');
        if ($search!='')
			 $query->where( $this->_db->quoteName('name_signature') . ' LIKE \'%' . $this->_db->escape($search) . '%\'' );
	...
	function setPublish(  $cid, $status ){
		if (count( $cid )) 
		{
			...
			$fields = array( $this->_db->quoteName('status_signature') . ' = ' . $this->_db->quote( $status ) );

			$conditions = array( $this->_db->quoteName('id_signature') . ' IN (' . $cids . ')' );
			
			$query->update($this->_db->quoteName('#__intranet_signature'))->set($fields)->where($conditions);
			...
			foreach($cid as $item)
				JLog::add($this->_user->get('id') . ' - ' . $text.' -  id item ('.$item.')',JLog::INFO, 'signature');
