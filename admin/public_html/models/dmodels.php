...
class EASistemasModelDModels extends JModelList
{
	var $_db = null;
	var $_user = null;
	var $_app = null;	
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        $this->context .'list.ordering','filter_order', 'id_documento', 'string');
    ...
    protected function getListQuery() {
		...
		$query->select($this->_db->quoteName(  array('id_documento',													 
													 //'U.name',
													 //'numero_documento',
													 //'ano_documento',
                                                     'name_documento',
													 ...
													 //'name_situacao',
													 //'sigla_estado',							 
													 '#__intranet_documento.checked_out',
													 '#__intranet_documento.checked_out_time',
													 )));	
									

		$query->select('A.name AS name_register_documento');											 						 
		$query->select('B.name AS name_update_documento');		
		$query->from($this->_db->quoteName('#__intranet_documento'));
		//$query->innerJoin($this->_db->quoteName('#__situacao').' USING ('.$this->_db->quoteName('id_situacao').')');
		$query->innerJoin($this->_db->quoteName('#__users').' A ON ('.$this->_db->quoteName('A.id').'='.$this->_db->quoteName('user_register_documento').')');
		$query->leftJoin($this->_db->quoteName('#__users').' B ON ('.$this->_db->quoteName('B.id').'='.$this->_db->quoteName('user_update_documento').')');
		

		$status = $this->getState('filter.status');
        if ($status!='')		
			 $query->where($this->_db->quoteName('status_documento') . '=' . $this->_db->quote($this->_db->escape($status)));
		else
			$query->where($this->_db->quoteName('status_documento') . '>=' . $this->_db->quote($this->_db->escape('0')));
        ...
        if ($search!='') {	 
			...
			$searches[]	= 'id_documento LIKE '.$token;
			//$searches[]	= 'name_situacao LIKE '.$token;
			$searches[]	= 'text_documento LIKE '.$token;
	...
	function setPublish(  $cid, $status ){
		if (count( $cid )) 
		{
			...
			foreach($cid as $item)
				JLog::add($this->_user->get('id') . ' - ' . $text.' -  id item ('.$item.')',JLog::INFO, 'documento');
