...
class EASistemasModelServiceTypes extends JModelList
{
	var $_db = null;
	var $_user = null;
	var $_app = null;	
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        $this->context .'list.ordering','filter_order', 'id_service_type', 'string');
    ...
    protected function getListQuery() {
		...
		$query->select($this->_db->quoteName(  array('id_service_type',
													 'status_service_type',
													 'name_service_type',
													 'checked_out',
													 'checked_out_time',
        ...
        if ($status!='')
			$query->where($this->_db->quoteName('status_service_type') . '=' . $this->_db->quote($this->_db->escape($status)));
		else
			$query->where($this->_db->quoteName('status_service_type') . '>=' . $this->_db->quote($this->_db->escape('0')));
				 
			
		$search = $this->getState('filter.search');
        if ($search!='')
			 $query->where( $this->_db->quoteName('name_service_type') . ' LIKE \'%' . $this->_db->escape($search) . '%\'' );
	...
	function setPublish(  $cid, $status ){
		if (count( $cid )) 
		{
			...
			$fields = array( $this->_db->quoteName('status_service_type') . ' = ' . $this->_db->quote( $status ) );

			$conditions = array( $this->_db->quoteName('id_service_type') . ' IN (' . $cids . ')' );
			
			$query->update($this->_db->quoteName('#__intranet_service_type'))->set($fields)->where($conditions);
			...
			foreach($cid as $item)
				JLog::add($this->_user->get('id') . ' - ' . $text.' -  id item ('.$item.')', JLog::INFO, 'servicetype');
