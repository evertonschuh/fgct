jQuery(document).ready(function(){
	...
	jQuery('#estado_buscacep').change(function(){
		jQuery('#cidade_buscacep').html('<option value="0">'+Joomla.JText._('FBT_SCRIPT_SELET_LOADING')+'</option>');	
		jQuery.post('views/system/includes/dynamicselect.php', 
				...
				if (!validateform(jQuery(this).parents('.form-validate')))
					...
					jQuery.post('views/system/includes/buscaendereco.php', 
						  ...
						  function(valor){
							 ...
							 jQuery('#submithsearch').html('<span class="glyphicon glyphicon-search"></span>&nbsp;'+Joomla.JText._('FBT_GLOBAL_BUTTON_NEW_SEARCH'));
...
function resetModaBuscaCep(img){
	...
	jQuery('#cidade_buscacep').html('<option disabled="" selected="" class="default" value="">'+Joomla.JText._('FBT_GLOBAL_CIDADE_SELECT')+'</option>');
	jQuery('#submithsearch').html('<span class="glyphicon glyphicon-search"></span>&nbsp;'+Joomla.JText._('FBT_GLOBAL_BUTTON_SEARCH'));
	...
	jQuery(form).find('select').each(function(){
		if(jQuery(this).prop('required')){
			if(!jQuery(this).val()){
				...
				jQuery(this).after( '<p class="control-label">'+Joomla.JText._('FBT_SCRIPT_VALIDATE_ERROR_INPUT_REQUIRED')+'</p>' );
				error = true;
			}
		} 	
	});	
