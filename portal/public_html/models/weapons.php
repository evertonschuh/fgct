...
class EASistemasModelWeapons extends JModelList
{
	var $_db = null;
	var $_user = null;
	var $_app = null;	
    ...
    protected function getListQuery() {
		...
		$query->select($this->_db->quoteName(  array('id_arma',													 
													 'image_arma',
													 'numero_arma',
													 'name_especie',	
													 'name_calibre',	
													 ...
													 'registro_arma',
													 //'sigla_estado',							 
													 '#__intranet_arma.checked_out',
													 '#__intranet_arma.checked_out_time',
													 )));	
									

		//$query->select('A.name AS name_register_arma');											 						 
	//	$query->select('B.name AS name_update_arma');		
		//$query->select('IF(ISNULL(name_pf), razao_social_pj, name_pf) AS name_proprietario');		
		$query->from($this->_db->quoteName('#__intranet_arma'));
